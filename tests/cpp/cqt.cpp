...
class PrecomputedRecursiveCQTTest : public ::testing::Test {
 protected:
  ...
  void ExpectComplexMatrixNear(const ComplexMatD &expected,
                               const ComplexMatD &actual, T tolerance,
  ...
  static void LoadReferenceData(
    ...
    while (std::getline(infile, line)) {
      ...
      if (line.rfind("# START ", 0) == 0) {
        ...
        reading_data = true;
        reading_complex_2d =
            (current_key.find("cqt_complex") != std::string::npos);
        expected_rows = 0;
        expected_cols = 0;
        ...
        } else  // Handle 1D float/double data
        {
          if (current_key.find("_f") !=
            ...
            expected_data_d[current_key] =
                flat_data_buffer;  // Direct move/copy
          }
        }
  ...
  static void SetUpTestSuite() {
    try {
      ...
      std::cout
          << "PrecomputedRecursiveCQTTest: Reference data loaded successfully."
          << std::endl;
    } catch (const std::exception &e) {
      // Use ADD_FAILURE to report error without stopping other fixtures
      ADD_FAILURE()
          << "FATAL ERROR in PrecomputedRecursiveCQTTest::SetUpTestSuite: "
  ...
  const ComplexMatD &GetExpectedLibrosaCQT() {
    ...
    if (expected_librosa_cqt_d.empty())
      throw std::runtime_error(
          "Librosa CQT reference data not loaded or empty.");
  ...
  static std::vector<T> callFilterAndDownsampleBy2(
      const OmniDSP::CQTPlan<T> &plan, const std::vector<T> &signal,
      double current_sr) {
...
TEST_F(PrecomputedRecursiveCQTTest, ConstructorPrecomputation) {
  ...
  ASSERT_NO_THROW(plan_ptr = std::make_unique<OmniDSP::CQTPlan<T>>(
                      sample_rate, hop_length, lowest_freq, high_freq,
                      bins_per_octave, hannWindowCoeffsGenerator<T>,
  ...
  ASSERT_NEAR(plan.getSparsityThreshold(), sparsity_threshold_double,
              get_tolerance<T>());
  ...
  for (int i = 0; i < expected_octaves; ++i) {
    ...
    ASSERT_EQ(getOctaveSpectrumLens(plan)[i],
              getOctaveFFTLens(plan)[i] / 2 + 1);
...
TEST_F(PrecomputedRecursiveCQTTest, CalculateFirCoefficients) {
  using T = double;
  OmniDSP::CQTPlan<T> plan(sample_rate, hop_length, lowest_freq, high_freq,
                           bins_per_octave, hannWindowCoeffsGenerator<T>,
  ...
  // Test Float precision
  {
    ...
    for (size_t i = 0; i < len; ++i) {
      test_signal_combined[i] =
          ...
          T(0.5) *
              std::sin(2.0 * M_PI * freq_high * static_cast<double>(i) / sr);
    }
    OmniDSP::CQTPlan<T> plan(
        sr, 128, 400, 1600, 12, hannWindowCoeffsGenerator<T>,
    ...
    long long valid_len_signed =
        static_cast<long long>(len) -
        static_cast<long long>(DEFAULT_FIR_FILTER_ORDER_TEST) + 1;
    if (valid_len_signed > 0) {
      size_t valid_len = static_cast<size_t>(valid_len_signed);
      expected_len =
          (valid_len > 0) ? ((valid_len - 1) / 2 + 1) : 0;  // Expect 974
    }

    // This assertion is EXPECTED TO FAIL if the backend (IPP) returns 1024
    // samples
    ...
    // Compare against loaded reference data (will also fail if sizes don't
    // match)
    const VecF &expected_ref = GetExpectedF("expected_filter_downsample_f");
    T tolerance = get_tolerance<T>() * 10;
    ExpectRealVectorNear(expected_ref, result, tolerance,
...
TEST_F(PrecomputedRecursiveCQTTest, FullRecursiveCQT_Execute_vs_Librosa) {
  // Test Float precision first
  {
    ...
    std::vector<T> test_signal =
        generateSineWave<T>(cqt_signal_freq, cqt_signal_duration, sample_rate);
    OmniDSP::CQTPlan<T> plan(sample_rate, hop_length, lowest_freq, high_freq,
                             bins_per_octave, hannWindowCoeffsGenerator<T>,
    ...
    ASSERT_EQ(cqt_output.size(), expected_cqt.size())
        << "Float Bin count mismatch vs Librosa reference.";
    if (!expected_cqt.empty()) {
      ...
      ASSERT_EQ(cqt_output[0].size(), expected_frames)
          << "Float Frame count mismatch vs Librosa reference.";
    }
  }
...
TEST_F(PrecomputedRecursiveCQTTest, ParameterValidation) {
  ...
  ASSERT_NO_THROW(OmniDSP::CQTPlan<T>(
      ...
      dummy_gen, sparsity_threshold_double, DEFAULT_FIR_FILTER_ORDER_TEST));
  ASSERT_THROW(
      OmniDSP::CQTPlan<T>(0.0, hop_length, lowest_freq, high_freq,
                          bins_per_octave, dummy_gen, sparsity_threshold_double,
                          DEFAULT_FIR_FILTER_ORDER_TEST),
      std::invalid_argument);
  ASSERT_THROW(
      OmniDSP::CQTPlan<T>(sample_rate, 0, lowest_freq, high_freq,
                          bins_per_octave, dummy_gen, sparsity_threshold_double,
                          DEFAULT_FIR_FILTER_ORDER_TEST),
      std::invalid_argument);
  ASSERT_THROW(
      OmniDSP::CQTPlan<T>(sample_rate, hop_length, 0.0, high_freq,
                          bins_per_octave, dummy_gen, sparsity_threshold_double,
                          DEFAULT_FIR_FILTER_ORDER_TEST),
      std::invalid_argument);
  ASSERT_THROW(
      OmniDSP::CQTPlan<T>(sample_rate, hop_length, lowest_freq, lowest_freq,
                          bins_per_octave, dummy_gen, sparsity_threshold_double,
                          DEFAULT_FIR_FILTER_ORDER_TEST),
      std::invalid_argument);
  ASSERT_THROW(OmniDSP::CQTPlan<T>(sample_rate, hop_length, lowest_freq,
                                   sample_rate / 1.9, bins_per_octave,
                                   dummy_gen, sparsity_threshold_double,
                                   DEFAULT_FIR_FILTER_ORDER_TEST),
               std::invalid_argument);  // > Nyquist check
  ASSERT_THROW(
      OmniDSP::CQTPlan<T>(sample_rate, hop_length, lowest_freq, high_freq, 0,
                          dummy_gen, sparsity_threshold_double,
                          DEFAULT_FIR_FILTER_ORDER_TEST),
      std::invalid_argument);
  ASSERT_THROW(
      OmniDSP::CQTPlan<T>(sample_rate, hop_length, lowest_freq, high_freq,
                          bins_per_octave, nullptr, sparsity_threshold_double,
                          DEFAULT_FIR_FILTER_ORDER_TEST),
      std::invalid_argument);
  ASSERT_THROW(
      OmniDSP::CQTPlan<T>(sample_rate, 511, lowest_freq, high_freq,
                          bins_per_octave, dummy_gen, sparsity_threshold_double,
                          DEFAULT_FIR_FILTER_ORDER_TEST),
      std::invalid_argument);  // Invalid hop
  ASSERT_THROW(OmniDSP::CQTPlan<T>(sample_rate, hop_length, lowest_freq,
                                   high_freq, bins_per_octave, dummy_gen, -0.1,
                                   DEFAULT_FIR_FILTER_ORDER_TEST),
               std::invalid_argument);  // Negative sparsity
  ASSERT_THROW(OmniDSP::CQTPlan<T>(sample_rate, hop_length, lowest_freq,
                                   high_freq, bins_per_octave, dummy_gen,

// tests/fft.cpp
// Tests for forward Complex-to-Complex FFT functions (fft, fft_inplace,
// FFTPlan::execute)
...
class FFT_C2C_Forward_Test : public ::testing::Test {
 protected:
  ...
  const size_t N = 16;
  const double double_tolerance = std::numeric_limits<double>::epsilon() * N *
                                  10;  // Looser tolerance for FFT
...
TEST_F(FFT_C2C_Forward_Test, InPlace_Double) {
  ...
  for (size_t i = 0; i < N; ++i)
    input[i] = Complex(i, -static_cast<double>(i) / 2.0);
  std::vector<Complex> input_copy =
      input;  // Keep original for comparison later if needed
  ...
  try {
    ...
    ExpectComplexVectorNear(spectrum_oop, input, double_tolerance,
                            "fft_inplace");

...
template <typename T>
void ExpectVectorNear(const std::vector<T>& expected, const std::vector<T>& actual, T tolerance) {
...
protected:
    ...
    // Kaiser: Requires Bessel function, harder to precompute simply. Test shape/endpoints.
    // beta = 8.0
    double kaiser_beta = 8.0;
    double i0_beta = std::cyl_bessel_i(0.0, kaiser_beta);
    std::vector<double> kaiser_coeffs_d = {
        std::cyl_bessel_i(0.0, kaiser_beta * sqrt(1.0 - pow(2.0*0/4.0 - 1.0, 2))) / i0_beta,
        std::cyl_bessel_i(0.0, kaiser_beta * sqrt(1.0 - pow(2.0*1/4.0 - 1.0, 2))) / i0_beta,
        std::cyl_bessel_i(0.0, kaiser_beta * sqrt(1.0 - pow(2.0*2/4.0 - 1.0, 2))) / i0_beta,
        std::cyl_bessel_i(0.0, kaiser_beta * sqrt(1.0 - pow(2.0*3/4.0 - 1.0, 2))) / i0_beta,
        std::cyl_bessel_i(0.0, kaiser_beta * sqrt(1.0 - pow(2.0*4/4.0 - 1.0, 2))) / i0_beta
    };
    std::vector<float> kaiser_coeffs_f; // Calculate from double

    // Flattop: Use coefficients a0-a4
    double ft_a0 = 0.21557895, ft_a1 = 0.41663158, ft_a2 = 0.277263158, ft_a3 = 0.083578947, ft_a4 = 0.006947368;
    std::vector<double> flattop_coeffs_d = {
        ft_a0 - ft_a1*cos(2*M_PI*0/4.) + ft_a2*cos(4*M_PI*0/4.) - ft_a3*cos(6*M_PI*0/4.) + ft_a4*cos(8*M_PI*0/4.),
        ft_a0 - ft_a1*cos(2*M_PI*1/4.) + ft_a2*cos(4*M_PI*1/4.) - ft_a3*cos(6*M_PI*1/4.) + ft_a4*cos(8*M_PI*1/4.),
        ft_a0 - ft_a1*cos(2*M_PI*2/4.) + ft_a2*cos(4*M_PI*2/4.) - ft_a3*cos(6*M_PI*2/4.) + ft_a4*cos(8*M_PI*2/4.),
        ft_a0 - ft_a1*cos(2*M_PI*3/4.) + ft_a2*cos(4*M_PI*3/4.) - ft_a3*cos(6*M_PI*3/4.) + ft_a4*cos(8*M_PI*3/4.),
        ft_a0 - ft_a1*cos(2*M_PI*4/4.) + ft_a2*cos(4*M_PI*4/4.) - ft_a3*cos(6*M_PI*4/4.) + ft_a4*cos(8*M_PI*4/4.)
    };
    ...
    void SetUp() override {
        ...
        std::transform(hamming_coeffs_d.begin(), hamming_coeffs_d.end(), hamming_coeffs_f.begin(),
                       [](double d){ return static_cast<float>(d); });

        kaiser_coeffs_f.resize(N);
        std::transform(kaiser_coeffs_d.begin(), kaiser_coeffs_d.end(), kaiser_coeffs_f.begin(),
                       [](double d){ return static_cast<float>(d); });

        flattop_coeffs_f.resize(N);
        std::transform(flattop_coeffs_d.begin(), flattop_coeffs_d.end(), flattop_coeffs_f.begin(),
                       [](double d){ return static_cast<float>(d); });

        // Calculate expected windowed signals
        expected_hann_f.resize(N); expected_hann_d.resize(N);
        expected_hamming_f.resize(N); expected_hamming_d.resize(N);
        expected_kaiser_f.resize(N); expected_kaiser_d.resize(N);
        expected_flattop_f.resize(N); expected_flattop_d.resize(N);
...
TEST_F(WindowTest, Kaiser) {
    auto result_f = OmniDSP::Window::kaiser(signal_f, static_cast<float>(kaiser_beta));
...
TEST_F(WindowTest, EdgeCases) {
    ...
    EXPECT_EQ(OmniDSP::Window::kaiser(single_f, 8.0f)[0], 1.0f); // Beta doesn't matter for N=1

// tests/irfft.cpp
// Tests for inverse Complex-to-Real FFT functions (irfft,
// FFTPlan::execute_irfft)
...
class FFT_C2R_Inverse_Test : public ::testing::Test {
 protected:
  ...
  std::vector<T> generate_real_signal(size_t len) {
    ...
    // Or use std::iota: std::iota(signal.begin(), signal.end(),
    // static_cast<T>(0));
    return signal;
  }
};
...
TEST_F(FFT_C2R_Inverse_Test, Identity_BackwardNorm_Double) {
  ...
  try {
    ...
    ExpectRealVectorNear(original, reconstructed, double_tolerance,
                         "rfft/irfft BACKWARD");
...
TEST_F(FFT_C2R_Inverse_Test, Identity_OrthoNorm_Float) {
  ...
  std::vector<Real> reconstructed(N);
  const float tol =
      float_tolerance * 10;  // Use slightly looser tolerance maybe
...
TEST_F(FFT_C2R_Inverse_Test, KnownTransform_RealDC_Double) {
  ...
  try {
    ...
    ExpectRealVectorNear(expected, output, double_tolerance,
                         "irfft(DC Spectrum)");
  } catch (const std::exception& e) {
...
// - Test with input spectrum that does NOT have Hermitian symmetry? (Output
// should not be purely real) - Low priority

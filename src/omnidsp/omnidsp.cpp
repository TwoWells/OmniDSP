...
class OmniDSPImpl {
 public:
  ...
  // --- One-off FFTs ---
  template <typename T>
  [[nodiscard]] virtual OmniExpected<std::vector<ComplexT<T>>> fft(  // Added
      const std::vector<ComplexT<T>>& input) const = 0;

  template <typename T>
  [[nodiscard]] virtual OmniExpected<std::vector<ComplexT<T>>> ifft(  // Added
      const std::vector<ComplexT<T>>& input) const = 0;

  template <typename T>
  [[nodiscard]] virtual OmniExpected<std::vector<ComplexT<T>>> rfft(  // Added
  ...
  create_cqt_plan(RealT<T> sample_rate, RealT<T> min_freq, RealT<T> max_freq,
...
[[nodiscard]] /* static */ OmniExpected<OmniDSP> OmniDSP::create(
  ...
  try {
...
#else
        std::cerr << "Warning: oneMKL backend requested but not enabled during "
    ...
    // Check if pimpl was successfully created (make_unique throws on allocation
    // failure)
    if (!pimpl) {
    ...
    std::cerr << "Error: Exception during backend initialization: " << e.what()
              << std::endl;                        // Error message
    return std::unexpected(Status::BackendError);  // Or a more specific error?
  } catch (...) {
...
// Move Assignment Operator: Needs to be defined here. = default is sufficient.
...
// --- DSP Operations ---
template <typename T>
[[nodiscard]] OmniExpected<std::vector<RealT<T>>> OmniDSP::convolve(  // Renamed
    const std::vector<RealT<T>>& input, const std::vector<RealT<T>>& kernel,
...
[[nodiscard]] OmniExpected<std::vector<RealT<T>>> OmniDSP::rectangular_window(
    size_t length) const {
  if (!pimpl_)
    throw std::runtime_error("Invalid OmniDSP instance in rectangular_window.");
...
[[nodiscard]] OmniExpected<std::vector<RealT<T>>> OmniDSP::triangular_window(
    size_t length) const {
  if (!pimpl_)
    throw std::runtime_error("Invalid OmniDSP instance in triangular_window.");
...
OmniDSP::create_cqt_plan(RealT<T> sample_rate, RealT<T> min_freq,
                         RealT<T> max_freq, int bins_per_octave) const {
  if (!pimpl_)
    throw std::runtime_error("Invalid OmniDSP instance in create_cqt_plan.");
  return pimpl_->create_cqt_plan<T>(sample_rate, min_freq, max_freq,
                                    bins_per_octave);
}

template <typename T>
[[nodiscard]] OmniExpected<std::unique_ptr<ResamplePlan<T>>>
OmniDSP::create_resample_plan(double input_rate, double output_rate,
                              size_t max_input_size) const {
  if (!pimpl_)
    throw std::runtime_error(
        "Invalid OmniDSP instance in create_resample_plan.");
  return pimpl_->create_resample_plan<T>(input_rate, output_rate,
                                         max_input_size);
}
...
template OmniExpected<std::vector<double>> OmniDSP::blackman_window(
    size_t) const;
template OmniExpected<std::vector<float>> OmniDSP::flattop_window(size_t) const;
template OmniExpected<std::vector<double>> OmniDSP::flattop_window(
    size_t) const;
template OmniExpected<std::vector<float>> OmniDSP::gaussian_window(size_t,
                                                                   float) const;
template OmniExpected<std::vector<double>> OmniDSP::gaussian_window(
    size_t, double) const;
template OmniExpected<std::vector<float>> OmniDSP::hamming_window(size_t) const;
...
template OmniExpected<std::vector<float>> OmniDSP::kaiser_window(size_t,
                                                                 float) const;
template OmniExpected<std::vector<double>> OmniDSP::kaiser_window(size_t,
                                                                  double) const;
...
template OmniExpected<std::unique_ptr<CQTPlan<float>>> OmniDSP::create_cqt_plan(
    float, float, float, int) const;

 ...
 * Also includes explicit template instantiations for FFTPlan and convenience functions.
 */
...
#include <type_traits>
#include <cmath>
#include <OmniDSP/omnidsp_export.h> // Make sure export header is included

// NOTE: FFTPlan<T> class methods (constructor, destructor, execute*, getters)
//       are defined and instantiated in the conditionally compiled backend files.
//       This file defines convenience functions and the explicit class/function instantiations.
...
namespace OmniDSP
{
    ...
    void irfft(const std::vector<std::complex<T>> &complex_input, std::vector<T> &real_output)
    {
        ...
        if (Nc < 1)
        { // Should not happen if complex_input wasn't empty, but defensive check
            real_output.clear();
            return;
        }
    ...
    // Add OMNIDSP_EXPORT here to ensure these explicit instantiations are exported from DLL
    // C2C Functions

...
/**
 * @brief Performs a Complex-to-Complex forward FFT (out-of-place) using
 * NormMode::BACKWARD.
 */
template <typename T>
void fft(const std::vector<std::complex<T>> &input,
  ...
  FFTPlan<T> plan(N, prec, Direction::FORWARD, Domain::COMPLEX,
                  NormMode::BACKWARD);
  plan.execute(input.data(), output.data());
}

/**
 * @brief Performs a Complex-to-Complex inverse FFT (out-of-place) using
 * NormMode::BACKWARD.
 */
template <typename T>
void ifft(const std::vector<std::complex<T>> &input,
  ...
  // Create and execute plan using default NormMode::BACKWARD
  FFTPlan<T> plan(N, prec, Direction::INVERSE, Domain::COMPLEX,
                  NormMode::BACKWARD);
  plan.execute(input.data(), output.data());
}

/**
 * @brief Performs a Complex-to-Complex forward FFT (in-place) using
 * NormMode::BACKWARD.
 */
template <typename T>
...
/**
 * @brief Performs a Complex-to-Complex inverse FFT (in-place) using
 * NormMode::BACKWARD.
 */
template <typename T>
...
/**
 * @brief Performs a Real-to-Complex forward FFT (out-of-place) using
 * NormMode::BACKWARD.
 */
template <typename T>
void rfft(const std::vector<T> &real_input,
  ...
  // Create and execute plan using default NormMode::BACKWARD
  FFTPlan<T> plan(N, prec, Direction::FORWARD, Domain::REAL,
                  NormMode::BACKWARD);
  plan.execute_rfft(real_input.data(), complex_output.data());
}

/**
 * @brief Performs a Complex-to-Real inverse FFT (out-of-place) using
 * NormMode::BACKWARD.
 */
template <typename T>
void irfft(const std::vector<std::complex<T>> &complex_input,
  ...
  // Create and execute plan using default NormMode::BACKWARD
  FFTPlan<T> plan(N, prec, Direction::INVERSE, Domain::REAL,
                  NormMode::BACKWARD);
...
// C2C Functions
template OMNIDSP_EXPORT void fft<float>(
    const std::vector<std::complex<float>> &,
    std::vector<std::complex<float>> &);
template OMNIDSP_EXPORT void fft<double>(
    const std::vector<std::complex<double>> &,
    std::vector<std::complex<double>> &);
template OMNIDSP_EXPORT void ifft<float>(
    const std::vector<std::complex<float>> &,
    std::vector<std::complex<float>> &);
template OMNIDSP_EXPORT void ifft<double>(
    const std::vector<std::complex<double>> &,
    std::vector<std::complex<double>> &);
template OMNIDSP_EXPORT void fft_inplace<float>(
    std::vector<std::complex<float>> &);
template OMNIDSP_EXPORT void fft_inplace<double>(
    std::vector<std::complex<double>> &);
template OMNIDSP_EXPORT void ifft_inplace<float>(
    std::vector<std::complex<float>> &);
template OMNIDSP_EXPORT void ifft_inplace<double>(
    std::vector<std::complex<double>> &);
// R2C / C2R Functions
template OMNIDSP_EXPORT void rfft<float>(const std::vector<float> &,
                                         std::vector<std::complex<float>> &);
template OMNIDSP_EXPORT void rfft<double>(const std::vector<double> &,
                                          std::vector<std::complex<double>> &);
template OMNIDSP_EXPORT void irfft<float>(
    const std::vector<std::complex<float>> &, std::vector<float> &);
template OMNIDSP_EXPORT void irfft<double>(

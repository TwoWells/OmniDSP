...
namespace OmniDSP {
  namespace backend {
    ...
    // Helper function to check if a length is supported by Accelerate
    // interleaved DFT Based on documentation: f * 2^n where f={1,3,5,9,15} and
    // n is within limits. Max length 4096. This checks the 'Length' parameter
    // passed to CreateSetup, which is N for C2C and N/2 for R2C.
    bool is_accelerate_interleaved_dft_length_supported(size_t length_param)
    {
      ...
      // Check against the specific allowed lengths derived from the f * 2^n
      // formula
        ...
        : length_(length)
    {
      ...
      // Validate against supported lengths for interleaved DFT (Length param is
      // N for C2C)
      if (!is_accelerate_interleaved_dft_length_supported(length_)) {
        throw std::invalid_argument(
            "Unsupported FFT length " + std::to_string(length_)
      ...
      if constexpr (std::is_same_v<T, std::complex<float>>) {
        setup_ = vDSP_DFT_Interleaved_CreateSetup(
            nullptr,
            static_cast<vDSP_Length>(length_),
    ...
    AccelerateFFTPlanImpl<T>::~AccelerateFFTPlanImpl()
    {
      if (setup_) {
        ...
        std::cout
            << "Accelerate FFTPlanImpl (Interleaved) destroyed for length "
            << length_ << std::endl;  // Debug
      }
    }
        ...
        : length_(length)
    {
      ...
      if constexpr (std::is_same_v<T, float>) {
        setup_ = vDSP_DFT_Interleaved_CreateSetup(
            nullptr,
            static_cast<vDSP_Length>(length_over_2),
      ...
      if (!setup_) {
        throw std::runtime_error(
            "Failed to create Accelerate interleaved DFTSetup (RealToComplex) "
            "for "
            "length "
            + std::to_string(length_));
      }
    ...
    AccelerateRFFTPlanImpl<T>::~AccelerateRFFTPlanImpl()
    {
      if (setup_) {
        ...
        std::cout
            << "Accelerate RFFTPlanImpl (Interleaved) destroyed for length "
            << length_ << std::endl;  // Debug
      }
        ...
        std::span<const RealT<T>> input, std::span<ComplexT<T>> output) const
    {
      ...
      // Typically might need multiplication by 0.5? Let's assume unscaled for
      // now.
        ...
        std::span<const ComplexT<T>> input, std::span<RealT<T>> output) const
    {
      ...
      temp_packed_input[0]
          = {input[0].real(), input[N_over_2].real()};  // DC and Nyquist packed
      for (size_t k = 1; k < N_over_2; ++k) {
      ...
      // Output is cast from Real* to DSPComplex* (N reals treated as N/2
      // complex)
      if constexpr (std::is_same_v<T, float>) {
        vDSP_DFT_Interleaved_Execute(
            setup_,
            reinterpret_cast<const DSPComplex*>(temp_packed_input.data()),
            reinterpret_cast<DSPComplex*>(output.data()),
            vDSP_DFT_INVERSE);
      }
      else {  // double
      ...
      // Scaling: Inverse real DFTs often need scaling by 1/N or 1/(2N). Check
      // docs. Leave unscaled for now, user responsibility.

 ...
 * source files (e.g., onemkl/convolution.cpp, accelerate/resample.cpp) must implement.
 * It acts as the abstraction layer between the public API/core library logic
 * and the specific backend code.
 */
 ...
 namespace OmniDSP
 {
     // Forward declaration for FFTPlanImpl (though not strictly needed in this header itself)
 ...
 namespace Backend
 {
      ...
      * The backend handles kernel reversal internally if needed for convolution.
      * @return std::vector<T> The result of the operation (typically 'valid' mode output).
      * @throws std::runtime_error If backend execution fails or is not available (stub).
      * @throws std::invalid_argument If inputs are invalid (e.g., empty, or kernel longer than signal).
      */
     template <typename T>
      ...
      * using internal filter based on kernel length for some backends like IPP FixedPolyphase)
      ...
      * @throws std::runtime_error If backend execution fails, is not available (stub), or unsupported (e.g., double precision IPP).
     ...
     // The public API (`OmniDSP::Window`) applies these coefficients to the input signal.

...
namespace OmniDSP
{
    ...
    // This struct holds placeholder members but its constructor and methods throw errors.
    template <typename T>
    struct FFTPlanImpl
    {
        ...
        FFTPlanImpl(size_t len, Precision prec, Direction dir, Domain dom, NormMode norm)
        {
            ...
            std::string error_msg = "OmniDSP backend (MKL/Accelerate) not selected or available during build. ";
    ...
    // These definitions link the public FFTPlan class methods to the stub PIMPL implementation.
    // The constructor will call the throwing FFTPlanImpl constructor.
    ...
    void FFTPlan<T>::execute(const std::complex<T> *i, std::complex<T> *o) const {
        if (!pimpl_) throw std::runtime_error("Invalid FFTPlan (stub - moved-from?).");
    ...
    // They will return default values if called after the constructor throws (which shouldn't happen).
    ...
    // even when linking against the stub implementation.

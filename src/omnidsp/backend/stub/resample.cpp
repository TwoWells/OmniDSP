...
namespace Backend
{
    ...
    /**
     * @brief Stub implementation for combined FIR filtering and downsampling. Throws error.
     */
    template <typename T>
    std::vector<T> filter_and_downsample_impl(const std::vector<T> &signal, const std::vector<T> &kernel, int factor)
    {
        ...
        throw std::runtime_error("OmniDSP backend (MKL/Accelerate) not selected or available during build. Cannot perform filter+downsample.");

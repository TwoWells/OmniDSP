 ...
 * @brief Stub (Error) backend implementation for OmniDSP convolution/correlation.
 *
...
namespace Backend
{
    ...
    std::vector<T> convolve1d_impl(const std::vector<T> &signal, const std::vector<T> &kernel, bool use_correlation)
    {
        ...
        throw std::runtime_error("OmniDSP backend (MKL/Accelerate) not selected or available during build. Cannot perform convolution/correlation.");

 ...
 * @details Performs the butterfly operations for a given stage length (`len`).
 * This function is intended to be called within the HWY_NAMESPACE.
 * Uses scalar twiddle factor loading for simplicity/correctness, which
 * could be a bottleneck for further optimization.
 ...
 * (k=0..N/2-1).
 * @param inverse True if computing inverse FFT (applies conjugate twiddle).
 */
template <typename T>
HWY_NOINLINE void CooleyTukeyStage_HWY(
    std::complex<T>* HWY_RESTRICT data, size_t N, size_t len,
    const std::complex<T>* HWY_RESTRICT twiddles_base, bool inverse) {
...
/**
 * @brief Performs an in-place iterative Cooley-Tukey FFT using Highway SIMD.
 ...
 * @tparam T_Complex Complex floating-point type (e.g., std::complex<float>).
 ...
 * @param bit_reverse_indices Precomputed bit-reversal permutation indices.
 */
template <typename T_Complex>
void CooleyTukeyFFT_HWY(std::span<T_Complex> data, bool inverse,
  ...
  for (size_t len = 2; len <= N; len <<= 1) {
    CooleyTukeyStage_HWY<T_Real>(data.data(), N, len, twiddles.data(), inverse);
  }
}

/**
 * @brief Unpacks N/2 complex FFT results into N/2+1 RFFT format using Highway.
 * @details Implements the unpacking formula using SIMD complex arithmetic.
 * This function lives inside the HWY_NAMESPACE.
 * @tparam T The underlying real floating-point type (float or double).
 * @param packed_fft_ptr Pointer to the N/2 complex FFT result (input).
 * @param WN_twiddles_ptr Pointer to precomputed W_N^k factors (k=0..N/2-1).
...
HWY_NOINLINE void RFFT_Unpack_HWY(
    ...
    std::complex<T>* HWY_RESTRICT output_ptr, size_t N) {
  const hn::ScalableTag<T> d;
  ...
  const CplxV v_half = hn::Set(d, T{0.5}, T{0.0});  // Vector containing 0.5
  const CplxV v_j = hn::Set(d, T{0.0}, T{1.0});  // Vector containing j (0+1i)
  ...
  for (; k + hn::Lanes(d) <= N / 4 + 1; k += hn::Lanes(d)) {
    ...
    const CplxV fft_k =
        hn::LoadInterleaved2(d, reinterpret_cast<const T*>(packed_fft_ptr + k));
    // Need to load fft[N/2-k] in reverse order for SIMD. Use Gather or scalar
    // loop. Using scalar loading for simplicity here.
    std::vector<T> temp_N2mk_real(hn::Lanes(d)), temp_N2mk_imag(hn::Lanes(d));
    ...
    const CplxV fft_N2_minus_k =
        hn::InterleaveLower(d, hn::LoadU(d, temp_N2mk_real.data()),
    ...
    CplxV term2_twiddle;
    term2_twiddle.val[0] = hn::Neg(term2_times_neg_WNk.val[1]);  // -imag part
    ...
    // For simplicity, we might compute and store, then fix the middle element
    // later if N/4 is a multiple of lanes.
    CplxV out_N2mk =
    ...
    for (size_t lane = 0; lane < hn::Lanes(d); ++lane) {
      if (k + lane <
        ...
        output_ptr[N_over_2 - (k + lane)] =
            std::complex<T>{hn::ExtractLane(out_N2mk.val[0], lane),
  ...
  for (; k < N_over_2; ++k) {
    ...
    std::complex<T> term2_twiddle = std::complex<T>{0.0, -1.0} * term2 * WN_k;

    output_ptr[k] = static_cast<T>(0.5) * (term1 + term2_twiddle);
    // Check k != N/2-k before assigning symmetric part (only needed if N/4 was
    // hit)
    if (k < N_over_2 - k) {
...
/**
 * @brief Reconstructs full N-point complex spectrum from N/2+1 RFFT input using
 * Highway.
 * @details Handles DC and Nyquist components separately, then uses SIMD to fill
 * k and N-k indices using conjugate symmetry.
 * This function lives inside the HWY_NAMESPACE.
 * @tparam T The underlying real floating-point type (float or double).
...
HWY_NOINLINE void IRFFT_Pack_HWY(
    ...
    std::complex<T>* HWY_RESTRICT full_spectrum_ptr, size_t N) {
  const hn::ScalableTag<T> d;
  using CplxV = hn::Vec2<decltype(d)>;
  const size_t N_over_2 = N / 2;
  ...
  full_spectrum_ptr[N_over_2] = std::complex<T>{rfft_input_ptr[N_over_2].real(),
                                                T{0.0}};  // Nyquist is real
  ...
  for (; k + hn::Lanes(d) <= N_over_2; k += hn::Lanes(d)) {
    // Load input[k] block
    const CplxV input_k =
        hn::LoadInterleaved2(d, reinterpret_cast<const T*>(rfft_input_ptr + k));
    // Compute conjugate block
    const CplxV conj_input_k = hn::Conj(input_k);

    // Store input[k] at full_spectrum[k]
    hn::StoreInterleaved2(input_k, d,
                          reinterpret_cast<T*>(full_spectrum_ptr + k));
    // Store conj(input[k]) at full_spectrum[N-k] (needs scatter or scalar loop)
    // Using scalar store for simplicity
    for (size_t lane = 0; lane < hn::Lanes(d); ++lane) {
      full_spectrum_ptr[N - (k + lane)] =
          std::complex<T>{hn::ExtractLane(conj_input_k.val[0], lane),
...
/**
 * @brief Copies the real part of a complex array to a real array using Highway.
...
HWY_NOINLINE void IRFFT_RealCopy_HWY(
    ...
    T* HWY_RESTRICT real_output_ptr, size_t N) {
  const hn::ScalableTag<T> d;
  using V = hn::Vec<decltype(d)>;
  using CplxV = hn::Vec2<decltype(d)>;
  ...
  for (; i + hn::Lanes(d) <= N; i += hn::Lanes(d)) {
    ...
    hn::StoreU(
        real_vec, d,
        real_output_ptr + i);  // Use StoreU for potentially unaligned output
  }
...
void CFFT_F32_HWY(std::span<float_c> d, bool inv, const std::vector<float_c>& t,
                  const std::vector<size_t>& bi) {
  CooleyTukeyFFT_HWY(d, inv, t, bi);
}
void CFFT_F64_HWY(std::span<double_c> d, bool inv,
                  const std::vector<double_c>& t,
                  const std::vector<size_t>& bi) {
  CooleyTukeyFFT_HWY(d, inv, t, bi);
}
void RFFT_Unpack_F32_HWY(const float_c* p, const float_c* w, float_c* o,
                         size_t n) {
  RFFT_Unpack_HWY<float>(p, w, o, n);
}
void RFFT_Unpack_F64_HWY(const double_c* p, const double_c* w, double_c* o,
                         size_t n) {
...
void CFFT_F32_Export(float_c* d, size_t N, bool inv, const float_c* t,
                     const size_t* bi) {
  HWY_NAMESPACE::CFFT_F32_HWY({d, N}, inv, {t, N / 2}, {bi, N});
}
HWY_EXPORT(CFFT_F32_Export);
void CFFT_F64_Export(double_c* d, size_t N, bool inv, const double_c* t,
                     const size_t* bi) {
...
void RFFT_Unpack_F32_Export(const float_c* p, const float_c* w, float_c* o,
                            size_t n) {
  HWY_NAMESPACE::RFFT_Unpack_F32_HWY(p, w, o, n);
}
HWY_EXPORT(RFFT_Unpack_F32_Export);
void RFFT_Unpack_F64_Export(const double_c* p, const double_c* w, double_c* o,
                            size_t n) {
...
Status DispatchRFFTUnpack(std::span<const float_c> packed_fft,
                          const std::vector<float_c>& WN_twiddles,
                          std::span<float_c> output, size_t N) {
...
Status DispatchRFFTUnpack(std::span<const double_c> packed_fft,
                          const std::vector<double_c>& WN_twiddles,
                          std::span<double_c> output, size_t N) {
  if (packed_fft.empty()) return Status::Success;
...
Status DispatchIRFFTPack(std::span<const float_c> rfft_input,
                         std::span<float_c> full_spectrum, size_t N) {
...
Status DispatchIRFFTPack(std::span<const double_c> rfft_input,
                         std::span<double_c> full_spectrum, size_t N) {
  if (rfft_input.empty()) return Status::Success;
...
Status DispatchIRFFTRealCopy(std::span<const float_c> complex_input,
                             std::span<float> real_output, size_t N) {
...
Status DispatchIRFFTRealCopy(std::span<const double_c> complex_input,
                             std::span<double> real_output, size_t N) {
  if (complex_input.empty()) return Status::Success;
...
inline size_t reverse_bits(size_t n,
                           size_t num_bits) { /* ... implementation ... */
  size_t reversed_n = 0;
...
//--------------------------------------------------------------------------
template <typename T>
DefaultFFTPlanImpl<T>::DefaultFFTPlanImpl(size_t length)
    : length_(length) { /* ... constructor ... */
  if (length == 0) return;
...
template <typename T>
Status DefaultFFTPlanImpl<T>::fft(std::span<const T> input,
                                  std::span<T> output) const {
  if (input.size() != length_ || output.size() != length_)
    return Status::SizeMismatch;
  if (length_ == 0) return Status::Success;
  std::copy(input.begin(), input.end(), output.begin());
  return DispatchComplexFFT(output, false, twiddle_factors_,
                            bit_reverse_indices_);
}
template <typename T>
Status DefaultFFTPlanImpl<T>::ifft(std::span<const T> input,
                                   std::span<T> output) const {
  if (input.size() != length_ || output.size() != length_)
    return Status::SizeMismatch;
  if (length_ == 0) return Status::Success;
  std::copy(input.begin(), input.end(), output.begin());
  return DispatchComplexFFT(output, true, twiddle_factors_,
                            bit_reverse_indices_);
}
template <typename T>
...
//--------------------------------------------------------------------------
template <typename T>
DefaultRFFTPlanImpl<T>::DefaultRFFTPlanImpl(size_t length)
    : length_(length) { /* ... constructor ... */
  if (length == 0) return;
  if (!is_power_of_two(length_))
...
template <typename T>
Status DefaultRFFTPlanImpl<T>::rfft(std::span<const RealT<T>> input,
  ...
  Status status =
      DispatchComplexFFT(std::span<ComplexT<T>>(packed_input), false,
                         twiddle_factors_, bit_reverse_indices_);
  ...
  status = DispatchRFFTUnpack(std::span<const ComplexT<T>>(packed_input),
                              WN_twiddles, output, N);
  return status;
}

template <typename T>
Status DefaultRFFTPlanImpl<T>::irfft(std::span<const ComplexT<T>> input,
  ...
  status = DispatchComplexFFT(std::span<ComplexT<T>>(full_spectrum), true,
  ...
  status = DispatchIRFFTRealCopy(std::span<const ComplexT<T>>(full_spectrum),
                                 output, N);
  return status;
}
template <typename T>

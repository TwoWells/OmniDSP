...
class ResamplePlanImpl {
 public:
  ...
  virtual Status execute(std::span<const T> input,
                         std::span<T> output) const = 0;  // Made const
...
class CQTPlanImpl {
 public:
  ...
  virtual Status execute(std::span<const RealT<T>> input,
                         std::span<ComplexT<T>> output) const = 0;
...
class OmniDSPImpl {
 public:
  ...
  create_cqt_plan(RealT<T> sample_rate, RealT<T> min_freq, RealT<T> max_freq,
                  int bins_per_octave,
                  const WindowSpec<T>& window_spec = WindowSpec<T>()) const = 0;
  ...
  create_iir_filter_plan(
      const std::vector<SecondOrderSection<T>>& sos_coefficients) const = 0;

  // --- Internal Helper for CQTPlanImpl Creation ---
  template <typename T>  // T is real type
  [[nodiscard]] virtual OmniExpected<std::unique_ptr<CQTPlanImpl<T>>>
  create_cqt_plan_impl(RealT<T> sample_rate, RealT<T> min_freq,
                       RealT<T> max_freq, int bins_per_octave,
                       const WindowSpec<T>& window_spec) const = 0;
   ...
   */
  template <typename T>  // T is real type
  [[nodiscard]] virtual OmniExpected<std::vector<RealT<T>>> design_fir_filter(
      const FIRFilterSpec<T>& spec) const = 0;  // <<< Already present
   ...
   * @return An OmniExpected containing the SOS coefficients on success, or a
   * Status error code.
   */
  template <typename T>  // T is real type
  ...
  [[nodiscard]] virtual OmniExpected<std::vector<RealT<T>>> generate_window(
      const WindowSpec<T>& spec, size_t length) const = 0;

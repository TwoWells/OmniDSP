...
DefaultOmniDSPImpl::DefaultOmniDSPImpl() {
  ...
  std::cout << "Default Backend Initialized." << std::endl;  // Debug message
}
...
template <typename T>
[[nodiscard]] OmniExpected<std::vector<RealT<T>>> DefaultOmniDSPImpl::convolve(
...
DefaultOmniDSPImpl::convolve(const std::vector<ComplexT<T>>& input,
  ...
  try {
    ...
    std::cerr << "Default convolve (complex) error: " << e.what() << std::endl;
    return std::unexpected(Status::Failure);
  }
}
template <typename T>
[[nodiscard]] OmniExpected<std::vector<RealT<T>>> DefaultOmniDSPImpl::correlate(
    const std::vector<RealT<T>>& input, const std::vector<RealT<T>>& kernel,
    ConvolutionMode mode) const {
  try {
    ...
    std::cerr << "Default correlate (real) error: " << e.what() << std::endl;
    return std::unexpected(Status::Failure);
  }
}
template <typename T>
[[nodiscard]] OmniExpected<std::vector<ComplexT<T>>>
DefaultOmniDSPImpl::correlate(const std::vector<ComplexT<T>>& input,
                              const std::vector<ComplexT<T>>& kernel,
                              ConvolutionMode mode) const {
  try {
    ...
    std::cerr << "Default correlate (complex) error: " << e.what() << std::endl;
    return std::unexpected(Status::Failure);
  }
}
...
template <typename T>
[[nodiscard]] OmniExpected<std::vector<ComplexT<T>>> DefaultOmniDSPImpl::fft(
...
template <typename T>
[[nodiscard]] OmniExpected<std::vector<ComplexT<T>>> DefaultOmniDSPImpl::ifft(
    const std::vector<ComplexT<T>>& input) const {
  try {
    DefaultFFTPlanImpl<ComplexT<T>> plan(input.size());
    std::vector<ComplexT<T>> output(input.size());
    ...
    if (!output.empty()) {
      ...
      RealType scale =
          static_cast<RealType>(1.0) / static_cast<RealType>(output.size());
      for (auto& val : output) {
        val *= scale;
      }
    }
...
template <typename T>
[[nodiscard]] OmniExpected<std::vector<ComplexT<T>>> DefaultOmniDSPImpl::rfft(
...
[[nodiscard]] OmniExpected<std::vector<RealT<T>>> DefaultOmniDSPImpl::irfft(
  ...
  try {
    ...
    if (!output.empty()) {
      RealT<T> scale =
          static_cast<RealT<T>>(1.0) / static_cast<RealT<T>>(output.size());
      for (auto& val : output) {
        val *= scale;
      }
    }
...
DefaultOmniDSPImpl::bartlett_window(size_t length) const {
  ...
  for (size_t n = 0; n < length; ++n) {
    coeffs[n] = static_cast<RealT<T>>(1.0) -
                std::abs(static_cast<RealT<T>>(2.0) * static_cast<RealT<T>>(n) /
                             N_minus_1 -
...
DefaultOmniDSPImpl::blackman_window(size_t length) const {
  if (length == 0) return std::vector<RealT<T>>();
  if (length == 1) return std::vector<RealT<T>>(1, static_cast<RealT<T>>(1.0));
  std::vector<RealT<T>> coeffs(length);
  ...
  const RealT<T> factor1 = (N_minus_1 > static_cast<RealT<T>>(0.0))
...
DefaultOmniDSPImpl::flattop_window(size_t length) const {
  ...
  const RealT<T> N_minus_1 = static_cast<RealT<T>>(length - 1);
  const RealT<T> factor = (N_minus_1 > static_cast<RealT<T>>(0.0))
                              ? (static_cast<RealT<T>>(2.0 * M_PI) / N_minus_1)
                              : static_cast<RealT<T>>(0.0);
  for (size_t n = 0; n < length; ++n) {
    RealT<T> n_T = static_cast<RealT<T>>(n);
...
DefaultOmniDSPImpl::create_resample_plan(double input_rate, double output_rate,
...
DefaultOmniDSPImpl::create_convolution_plan(const std::vector<T>& kernel,
                                            ConvolutionMode mode) const {
  try {
    auto pimpl_backend =
...
DefaultOmniDSPImpl::create_correlation_plan(const std::vector<T>& kernel,
                                            ConvolutionMode mode) const {
  try {
    auto pimpl_backend =
 ...
 * @return An OmniExpected containing a unique_ptr to the public FIRFilterPlan
...
DefaultOmniDSPImpl::create_fir_filter_plan(
  ...
  try {
    ...
    // unique_ptr<FIRFilterPlanImpl<T>> and OmniDSP (or DefaultOmniDSPImpl) to
    // be a friend.
    FIRFilterPlan<T>* plan_ptr = new FIRFilterPlan<T>(std::move(pimpl_backend));
 ...
 * @param sos_coefficients A vector of Second-Order Sections representing the
 * IIR filter.
 * @return An OmniExpected containing a unique_ptr to the public IIRFilterPlan
 * on success, or a Status error code.
 */
template <typename T>
...
DefaultOmniDSPImpl::create_iir_filter_plan(
  ...
  try {
    ...
    // unique_ptr<IIRFilterPlanImpl<T>> and OmniDSP (or DefaultOmniDSPImpl) to
    // be a friend.
    IIRFilterPlan<T>* plan_ptr = new IIRFilterPlan<T>(std::move(pimpl_backend));
 ...
 * @return An OmniExpected containing the filter coefficients on success, or a
 * Status error code.
 */
template <typename T>
[[nodiscard]] OmniExpected<std::vector<RealT<T>>>
DefaultOmniDSPImpl::design_fir_filter(const FIRFilterSpec<T>& spec) const {
  ...
  for (size_t n = 0; n < num_taps; ++n) {
    ...
    switch (spec.type) {
        ...
        ideal_coeffs[n] =
            ((m == static_cast<RealT<T>>(0.0))
                 ...
                 : std::sin(static_cast<RealT<T>>(M_PI) * m) /
                       (static_cast<RealT<T>>(M_PI) * m))  // Sinc(n) for delta
      ...
      case FilterType::Bandpass: {
        ...
        RealT<T> sinc_val2 = (m == static_cast<RealT<T>>(0.0))
                                 ? static_cast<RealT<T>>(1.0)
                                 : std::sin(static_cast<RealT<T>>(2.0 * M_PI) *
                                            fn2.value() * m) /
                                       (static_cast<RealT<T>>(M_PI) * m);
        ideal_coeffs[n] = static_cast<RealT<T>>(2.0) * fn2.value() * sinc_val2 -
                          static_cast<RealT<T>>(2.0) * fn1 * sinc_val;
        break;
      }
      case FilterType::Bandstop: {
        ...
        ideal_coeffs[n] =
            ...
            (static_cast<RealT<T>>(2.0) * fn2.value() * sinc_val2 -
             static_cast<RealT<T>>(2.0) * fn1 * sinc_val);  // Subtract bandpass
        break;
      }
      default:
        return std::unexpected(Status::InvalidArgument);  // Unknown filter type
    }
  }
  ...
  if (spec.type == FilterType::Lowpass) {
    RealT<T> sum = std::accumulate(final_coeffs.begin(), final_coeffs.end(),
                                   static_cast<RealT<T>>(0.0));
    if (std::abs(sum) >
        static_cast<RealT<T>>(1e-9)) {  // Avoid division by zero/small number
...
DefaultOmniDSPImpl::design_iir_filter(const IIRFilterSpec<T>& spec) const {
  // TODO: Implement IIR filter design algorithms (Butterworth, Chebyshev I/II,
  // Elliptic) This involves complex steps like:
 ...
 * @return An OmniExpected containing the window coefficients on success, or a
 * Status error code.
 */
template <typename T>
[[nodiscard]] OmniExpected<std::vector<RealT<T>>>
DefaultOmniDSPImpl::generate_window(const WindowSpec<T>& spec,
                                    size_t length) const {
  // Dispatch to the specific window generation method based on spec.get_type()
  switch (spec.get_type()) {
    ...
    default:
      return std::unexpected(Status::InvalidArgument);  // Unknown window type
  }
}
...
//--------------------------------------------------------------------------
// Instantiate templates for common types (float, double) for DefaultOmniDSPImpl
// methods
...
// Window Generation
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::bartlett_window(
    size_t) const;
template OmniExpected<std::vector<double>> DefaultOmniDSPImpl::bartlett_window(
    size_t) const;
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::blackman_window(
    size_t) const;
template OmniExpected<std::vector<double>> DefaultOmniDSPImpl::blackman_window(
    size_t) const;
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::flattop_window(
    size_t) const;
template OmniExpected<std::vector<double>> DefaultOmniDSPImpl::flattop_window(
    size_t) const;
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::gaussian_window(
    size_t, float) const;
template OmniExpected<std::vector<double>> DefaultOmniDSPImpl::gaussian_window(
    size_t, double) const;
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::hamming_window(
    size_t) const;
template OmniExpected<std::vector<double>> DefaultOmniDSPImpl::hamming_window(
    size_t) const;
...
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::kaiser_window(
    size_t, float) const;
template OmniExpected<std::vector<double>> DefaultOmniDSPImpl::kaiser_window(
    size_t, double) const;
...
template OmniExpected<std::vector<double>>
    DefaultOmniDSPImpl::rectangular_window(size_t) const;
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::triangular_window(
    size_t) const;
template OmniExpected<std::vector<double>>
...
DefaultOmniDSPImpl::create_cqt_plan(const OmniDSP*, float, float, float,
                                    int) const;
template OmniExpected<std::unique_ptr<CQTPlan<double>>>
DefaultOmniDSPImpl::create_cqt_plan(const OmniDSP*, double, double, double,
                                    int) const;
...
DefaultOmniDSPImpl::create_convolution_plan(const std::vector<float>&,
                                            ConvolutionMode) const;
template OmniExpected<std::unique_ptr<ConvolutionPlan<double>>>
DefaultOmniDSPImpl::create_convolution_plan(const std::vector<double>&,
                                            ConvolutionMode) const;
template OmniExpected<std::unique_ptr<ConvolutionPlan<float_c>>>
DefaultOmniDSPImpl::create_convolution_plan(const std::vector<float_c>&,
                                            ConvolutionMode) const;
template OmniExpected<std::unique_ptr<ConvolutionPlan<double_c>>>
DefaultOmniDSPImpl::create_convolution_plan(const std::vector<double_c>&,
                                            ConvolutionMode) const;

// Correlation Plan Factories
template OmniExpected<std::unique_ptr<CorrelationPlan<float>>>
DefaultOmniDSPImpl::create_correlation_plan(const std::vector<float>&,
                                            ConvolutionMode) const;
template OmniExpected<std::unique_ptr<CorrelationPlan<double>>>
DefaultOmniDSPImpl::create_correlation_plan(const std::vector<double>&,
                                            ConvolutionMode) const;
template OmniExpected<std::unique_ptr<CorrelationPlan<float_c>>>
DefaultOmniDSPImpl::create_correlation_plan(const std::vector<float_c>&,
                                            ConvolutionMode) const;
template OmniExpected<std::unique_ptr<CorrelationPlan<double_c>>>
DefaultOmniDSPImpl::create_correlation_plan(const std::vector<double_c>&,
                                            ConvolutionMode) const;
...
DefaultOmniDSPImpl::create_fir_filter_plan(const std::vector<double>&) const;
template OmniExpected<std::unique_ptr<FIRFilterPlan<float_c>>>
DefaultOmniDSPImpl::create_fir_filter_plan(const std::vector<float_c>&) const;
template OmniExpected<std::unique_ptr<FIRFilterPlan<double_c>>>
DefaultOmniDSPImpl::create_fir_filter_plan(const std::vector<double_c>&) const;
...
// Filter Design Methods
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::design_fir_filter(
...
template OmniExpected<std::vector<float>> DefaultOmniDSPImpl::generate_window(
    const WindowSpec<float>&, size_t) const;
template OmniExpected<std::vector<double>> DefaultOmniDSPImpl::generate_window(

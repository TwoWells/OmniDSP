...
/**
 * @brief Concrete implementation of ResamplePlanImpl for the Default backend.
 * Uses standard C++ and polyphase FIR filtering.
 */
template <typename T>  // T is real type here
class DefaultResamplePlanImpl final : public ResamplePlanImpl<T> {
 public:
  /**
   * @brief Constructor. Calculates factors, designs filter, initializes state.
   ...
   * @throws std::runtime_error If factor calculation or filter design fails.
   */
  DefaultResamplePlanImpl(const OmniDSPImpl* owner, const ResampleSpec& spec);
 ...
 private:
  ...
  size_t current_phase_ = 0;     // Current phase index for polyphase processing
...
template <typename T>
DefaultResamplePlanImpl<T>::DefaultResamplePlanImpl(const OmniDSPImpl* owner,
                                                    const ResampleSpec& spec)
    : input_rate_(spec.input_rate), output_rate_(spec.output_rate) {
  if (!owner) {
    throw std::invalid_argument(
        "DefaultResamplePlanImpl requires a valid owner OmniDSPImpl pointer.");
  }
  if (!spec.validate()) {
  ...
  if (num_taps > 0) {
    ...
    // Handle case where filter design somehow resulted in zero taps (should be
    // error earlier)
    filter_state_.clear();
...
template <typename T>
Status DefaultResamplePlanImpl<T>::execute(std::span<const T> input,
                                           std::span<T> output) {
  ...
  work_buffer.insert(work_buffer.end(), filter_state_.begin(),
                     filter_state_.end());
  ...
  while (output_idx < output_len_required) {
    // Calculate the index in the *input* buffer corresponding to the *start*
    // of the filter application for the current output sample.
    // This requires knowing how many input samples are consumed per output
    ...
    // Calculate the index into the combined state+input buffer `work_buffer`
    // corresponding to the *first tap* of the *current* polyphase filter
    // branch.
    size_t work_buffer_base_idx =
        current_phase_ / L;  // Index of the input sample corresponding to phase
                             // 0 of this filter application

    // Check if we have enough data in work_buffer to apply the filter
    // The last needed sample is at work_buffer_base_idx + num_branch_taps - 1
    if (work_buffer_base_idx + num_branch_taps > work_buffer.size()) {
      // Not enough input samples + state to compute the full output length.
      // This can happen if output_len_required was slightly overestimated or
      // input ended. std::cerr << "Warning: Resampler execute stopping early.
      // Input/State exhausted." << std::endl;
    ...
    for (size_t n = 0; n < num_branch_taps; ++n) {
      size_t proto_idx = phase_index + n * L;  // Index into prototype filter
      if (proto_idx < num_taps) {
        ...
        // Access coefficient h[phase + n*L] and multiply by input x[floor(t/L)
        // + n]
  ...
  // samples from the portion of `work_buffer` that was used. The relevant part
  // of work_buffer ends at index `consumed_input_samples + num_branch_taps - 1`
  // ?? No. Simpler: The state should be the last `state_len` samples of the
  // *original* input that were involved. Let's find the index in the *original
  // input span* corresponding to the start of the state needed. Total samples
  // processed in work_buffer = consumed_input_samples + (num_taps/L) approx?

  // Easier way: The state is the last `state_len` elements of the `work_buffer`
  // that were *potentially* accessed. The highest index accessed in work_buffer
  // was roughly `work_buffer_base_idx + num_branch_taps - 1` where
  // work_buffer_base_idx was `(current_phase_ - M) / L` at the last step. Let's
  // just copy the *last* `state_len` elements from the `work_buffer`.
  size_t copy_start_idx =
      (work_buffer.size() >= state_len) ? (work_buffer.size() - state_len) : 0;
  size_t num_to_copy = std::min(state_len, work_buffer.size());

  if (state_len > 0 && num_to_copy > 0) {
    std::copy(work_buffer.begin() + copy_start_idx,
    ...
    if (num_to_copy < state_len) {
      std::fill(filter_state_.begin() + num_to_copy, filter_state_.end(), T{0});
    }
  }
...
size_t DefaultResamplePlanImpl<T>::get_output_length(
  ...
  // safe. Need to account for the state and phase. Effective input length =
  // input_length + state_len Effective output samples = floor((input_length * L
  ...
  size_t filter_contribution = (prototype_filter_coeffs_.size() + L - 1) /
...
template <typename T>
Status DefaultResamplePlanImpl<T>::calculate_factors_static(double in_rate,
                                                            double out_rate,
                                                            size_t& L,
                                                            size_t& M) {
  ...
  if (common == 0) return Status::Failure;  // Should not happen if rates > 0
...
template <typename T>
Status DefaultResamplePlanImpl<T>::design_filter(const OmniDSPImpl* owner,
                                                 const ResampleSpec& spec,
                                                 size_t L, size_t M) {
  // Use the base class implementation provided in backend.h (via previous step)
  // This requires the implementation to be available, typically by defining it
  // in a shared .cpp file or ensuring backend.h includes its definition source.
  // Assuming the definition from the previous step is accessible:

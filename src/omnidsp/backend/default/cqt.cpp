  ...
  namespace backend {
    ...
    inline size_t next_power_of_two(size_t n)
    {
      ...
      while (power < n) {
        power <<= 1;
        if (power == 0)
          return std::numeric_limits<size_t>::max();  // Check overflow during
                                                      // shift
      }
      return power;
    }
    ...
    class DefaultCQTPlanImpl final : public CQTPlanImpl<T> {
     public:
       ...
       * @param owner_impl Pointer to the OmniDSPImpl instance creating this
       * plan.
       ...
       * kernels.
       * @throws std::invalid_argument If parameters are invalid or owner_impl
       * is null.
          ...
          window_spec_(window_spec)
    {
      ...
      std::cout
          << "Default CQTPlanImpl Constructor (Recursive): Setting up CQT..."
          << std::endl;

      // --- 1. Calculate Global CQT Parameters ---
      quality_factor_ = static_cast<RealT<T>>(1.0)
                        / (std::pow(
                               static_cast<RealT<T>>(2.0),
      ...
      for (size_t k = 0; k < num_bins_; ++k) {
        ...
        if (center_frequencies_[k]
            >= sample_rate_ / static_cast<RealT<T>>(2.0)) {
      ...
      // A smaller hop length increases temporal resolution but also
      // computation. Common choices are powers of 2, or related to kernel
      // lengths. Using kernel_lengths_[0] / 4 as a starting point.
      hop_length_ = std::max(static_cast<size_t>(1), kernel_lengths_[0] / 4);
      // Ensure hop_length is reasonable, maybe power of 2?
      // hop_length_ = next_power_of_two(hop_length_); // Optional: force power
      // of 2
      ...
      auto fft_plan_result
          = owner_impl_->create_fft_plan<ComplexT<T>>(fft_length_);
      if (!fft_plan_result) {
      ...
      // Create the ResampleSpec for factor-2 downsampling. Use a default
      // quality.
      ...
      // resample_spec.max_input_size = 0; // Optional hint, not used in current
      // API
      ...
      std::cout
          << "  Internal Resample Plan (Factor 2) created via owner backend."
          << std::endl;
      ...
      for (size_t k = 0; k < num_bins_; ++k) {
        ...
        auto window_coeffs_result
            = owner_impl_->generate_window(window_spec_, N_k);
      ...
      std::cout
          << "Default CQTPlanImpl Constructor (Recursive): Setup complete."
          << std::endl;
    }
        ...
        std::span<const RealT<T>> input, std::span<ComplexT<T>> output) const
    {
                               ...
                               size_t bin_offset) -> Status
      {
        ...
        // rate. These are the bins whose center frequencies are >=
        // current_nyquist / 2 and < current_nyquist.
        ...
        start_bin
            = end_bin;  // Start processing from the first bin in this octave
        while (end_bin < num_bins_
        ...
        if (num_bins_this_octave > 0) {
          ...
          size_t current_n_frames
              = (current_input.size() + current_hop_length - 1)
                / current_hop_length;
          if (current_n_frames == 0 && !current_input.empty())
            current_n_frames = 1;

          // Ensure the number of frames matches the total expected frames
          // This recursive approach might lead to slight variations if not
          // careful. For simplicity, assume current_n_frames aligns with
          // n_frames_total for now. A more robust implementation might need
          // padding or careful frame alignment.
          if (current_n_frames != n_frames_total) {
            std::cerr << "Warning: Frame count mismatch in CQT recursion ("
                      ...
                      << std::endl;
            // Adjust current_n_frames to match total? Or handle potential
          ...
          for (size_t i = 0; i < current_n_frames; ++i) {
            ...
            std::fill(
                frame_buffer.begin(), frame_buffer.end(), ComplexT<T>{0, 0});
            for (size_t n = 0; n < current_frame_len; ++n) {
              frame_buffer[n]
                  = ComplexT<T>{current_input[frame_start + n], 0.0};
            }
            ...
            for (size_t k_idx = 0; k_idx < num_bins_this_octave; ++k_idx) {
              ...
              for (size_t f = 0; f < fft_length_; ++f) {
                cqt_coeff
                    += frame_fft[f]
                       * kernels_fft_[k]
                                     [f];  // kernels_fft_ is already conjugated
              }
              // Store in the final output buffer at the correct frame and bin
              // index Output layout: [bin0_frame0, bin1_frame0, ...,
              // binK_frame0, bin0_frame1, ...] (Column-major: bins vary
              // fastest) size_t output_idx = k * n_frames_total + i; //
              // Column-major Output layout: [bin0_frame0, bin0_frame1, ...,
              // bin0_frameN, bin1_frame0, ...] (Row-major: frames vary fastest)
              size_t output_idx
                  = i * n_bins_total + k;  // Row-major (frame by frame - seems
                                           // more common for spectrograms)

              if (output_idx < output.size()) {
                // Apply scaling factor (1/N from IFFT implicit in convolution)
                output[output_idx]
                    = cqt_coeff / static_cast<RealT<T>>(fft_length_);
              }
              else {
          ...
          // std::cout << "  No bins to process for SR=" << current_sr <<
          // std::endl;
          // // Debug
        }
        ...
        if (lowest_bin_processed
          ...
          // std::cout << "  Downsampling for next octave..." << std::endl; //
          // Debug
          // 4. Downsample the signal
          // Need to reset the resampler state before processing each octave
          // level? Yes.
          ...
          // from estimate. Using the vector's actual size after resizing might
          // be safer if execute doesn't guarantee filling. Assuming execute
          // fills the span up to the required length.

          // 5. Recursive call for the lower octave
          // The bin_offset remains 0 because we process *all remaining* bins at
        ...
        size_t input_length) const
    {
      ...
      // Calculate frames based on the hop length defined at the *original*
      // sample rate.
      return (input_length + hop_length_ - 1)

...
namespace OmniDSP {
namespace backend {
// namespace HWY_NAMESPACE { // Start Highway's target-specific namespace (if
// Highway used)
...
DefaultFIRFilterPlanImpl<T>::DefaultFIRFilterPlanImpl(
    ...
    : coefficients_(coefficients),
      state_(coefficients.empty() ? 0 : coefficients.size() - 1,
...
template <typename T>
DefaultFIRFilterPlanImpl<T>::~DefaultFIRFilterPlanImpl() {
  std::cout << "Default FIRFilterPlanImpl destroyed." << std::endl;  // Debug
}
 ...
 * @param output The output signal buffer. Must be at least as large as input.
 ...
 */
template <typename T>
Status DefaultFIRFilterPlanImpl<T>::execute(std::span<const T> input,
                                            std::span<T> output) {
  if (coefficients_.empty()) {
    std::fill(output.begin(), output.end(), T{0});
    // Return success or an error? Let's assume success as it produces defined
    // output (zeros) If empty coefficients are invalid, the constructor should
    // have thrown.
  ...
  if (input.empty()) {
    std::fill(output.begin(), output.begin() + output.size(),
  ...
  // 1. Direct Convolution: Simple loop, suitable for short filters. Implemented
  // below.
  // 2. FFT-based Convolution (Overlap-Add / Overlap-Save): More efficient for
  // long filters. (Placeholder)
  ...
  // Create a temporary working buffer combining the previous state and the new
  // input This avoids complex indexing into separate state/input buffers during
  // convolution.
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    T result{};
    // The convolution sum for output[i] involves input samples from x[i] down
    // to x[i - num_taps + 1] In the work_buffer, this corresponds to indices
    // from (state_len + i) down to (state_len + i - num_taps + 1)
    size_t work_buffer_start_idx = state_len + i;

    // Perform the dot product (convolution sum): y[i] = sum_{k=0}^{num_taps-1}
    // h[k] * x[i-k] Where x refers to samples in the conceptual infinite signal
    // (represented by work_buffer)
  ...
  // work_buffer
  if (state_len > 0) {
    // Ensure work_buffer has enough elements (should always be true if input is
    // not empty)
    if (work_buffer.size() >= state_len) {
      std::copy(work_buffer.end() - state_len, work_buffer.end(),
                state_.begin());
    } else {
      // This case indicates a logic error or unexpected empty input scenario
  ...
  //     std::cerr << "FFT-based FIR filtering not yet implemented in Default
  //     backend." << std::endl; return Status::NotImplemented;
  // }
 ...
 * @return The filter order (number of taps - 1). Returns 0 if coefficients are
 * empty.
 */
template <typename T>
...
template <typename T>
DefaultIIRFilterPlanImpl<T>::~DefaultIIRFilterPlanImpl() {
  std::cout << "Default IIRFilterPlanImpl destroyed." << std::endl;  // Debug
}
 ...
 */
template <typename T>
Status DefaultIIRFilterPlanImpl<T>::execute(std::span<const T> input,
                                            std::span<T> output) {
  if (sos_coeffs_.empty()) {
    std::fill(output.begin(), output.end(), T{0});
  ...
  //     section's feed-forward part state1_k[n] = input_to_section * b1_k - y_n
  //     * a1_k + state2_k[n-1] // Update state 1 state2_k[n] = input_to_section
  //     * b2_k - y_n * a2_k               // Update state 2 input_to_section =
  //     y_n // Output of this section is input to next
  ...
  for (size_t n = 0; n < input.size(); ++n) {
    ...
    for (size_t k = 0; k < num_sections; ++k) {
      ...
      T& s1 = state_[k * 2 + 0];  // Reference to state variable 1 (delay z^-1)
                                  // for section k
      T& s2 = state_[k * 2 + 1];  // Reference to state variable 2 (delay z^-2)
                                  // for section k
...
/**
 * @brief Resets the internal state of the IIR filter (delay elements) to zeros.
 * @return Status::Success.
 */
template <typename T>

...
namespace Backend
{
    ...
    template <typename T> struct vDSPDesampHelper;
    template <> struct vDSPDesampHelper<float>  { static constexpr auto desamp = vDSP_desamp; }; // vDSP_desamp for float
    template <> struct vDSPDesampHelper<double> { static constexpr auto desamp = vDSP_desampD; }; // vDSP_desampD for double

    /**
     * @brief Accelerate backend implementation for combined FIR filtering and downsampling.
     * Uses vDSP_desamp which performs both operations efficiently using the provided kernel.
     *
     ...
     * @throws std::invalid_argument If inputs are invalid (empty, factor <= 0, kernel empty).
     */
    template <typename T>
    std::vector<T> filter_and_downsample_impl(const std::vector<T> &signal, const std::vector<T> &kernel, int factor)
    {
        ...
        // Signature: vDSP_desamp(InputSignal, DecimationFactor, FilterCoeffs, Output, OutputLength, FilterLength)
        // vDSP Documentation uses: (A, D, F, C, N, L)

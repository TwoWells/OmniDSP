...
class AccelerateFFTPlanImpl final : public FFTPlanImpl<T> {
 ...
 private:
  ...
  typename std::conditional<std::is_same_v<T, std::complex<float>>,
                            vDSP_DFT_Setup, vDSP_DFT_SetupD>::type setup_;
...
class AccelerateRFFTPlanImpl final : public RFFTPlanImpl<T> {
 ...
 private:
  ...
  typename std::conditional<std::is_same_v<T, float>, vDSP_FFTSetup,
                            vDSP_FFTSetupD>::type setup_;
  // Temporary buffer often needed for vDSP real FFTs
  typename std::conditional<std::is_same_v<T, float>, DSPSplitComplex,
...
class AccelerateResamplePlanImpl final : public ResamplePlanImpl<T> {
 public:
  ...
  AccelerateResamplePlanImpl(double input_rate, double output_rate,
...
class AccelerateConvolutionPlanImpl final : public ConvolutionPlanImpl<T> {
 public:
  ...
  AccelerateConvolutionPlanImpl(const std::vector<T>& kernel,
 ...
 private:
  ...
  // Potentially store FFT setup and FFT'd kernel if using FFT-based convolution
  // std::unique_ptr<AccelerateFFTPlanImpl<T>> fft_plan_; // Example
  // std::vector<T> kernel_fft_; // Example
};
...
class AccelerateCorrelationPlanImpl final : public CorrelationPlanImpl<T> {
 public:
  ...
  AccelerateCorrelationPlanImpl(const std::vector<T>& kernel,
                                ConvolutionMode mode);
  // Destructor

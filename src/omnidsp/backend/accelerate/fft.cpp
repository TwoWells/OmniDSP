 ...
 * Apple's Accelerate framework (vDSP) routines (vDSP_DFT for C2C, vDSP_fft for Real).
...
namespace OmniDSP
{
    ...
    struct FFTPlanImpl
    {
        ...
        std::vector<T> real_buffer; // Temporary buffer for real part in Real FFTs (split complex)
        std::vector<T> imag_buffer; // Temporary buffer for imag part in Real FFTs (split complex)
        ...
        // It also currently requires N to be a power of 2 for REAL domain FFTs using vDSP_fft_zrip.
        vDSP_Length get_log2n(size_t n)
        {
            ...
            // Use cast to vDSP_Length which is typically unsigned long
            return (domain == Domain::REAL && n != 0) ? static_cast<vDSP_Length>(std::log2(static_cast<double>(n))) : 0;
        }
        ...
        FFTPlanImpl(size_t len, Precision prec, Direction dir, Domain dom, NormMode norm)
        {
            ...
            if (dom == Domain::COMPLEX)
            {
                ...
                // Determine scaling factors based on mode (vDSP_DFT does NOT scale internally)
                switch (norm_mode)
                {
            ...
            else // Domain::REAL
            {
                ...
                T factor = static_cast<T>(1.0) / implicit_inv_scale; // Factor to multiply vDSP result by to get unscaled
                T factorSqrt = std::sqrt(factor);

                if (factorSqrt == static_cast<T>(0.0) && norm_mode == NormMode::ORTHO && scaleN > 0) // Defensive check
                {
                ...
                case NormMode::BACKWARD: // RFFT unscaled(1.0), IRFFT scaled by 1/N
                    ...
                    backward_scale = (1.0 / scaleN) * factor; // Apply (1/N) * (N/2) = 1/2 to get 1/N overall inverse
                    break;
                case NormMode::ORTHO: // RFFT & IRFFT scaled by 1/sqrt(N)
                    forward_scale = (1.0 / scaleSqrtN) * factor; // Apply (1/sqrtN) * (N/2)
                    backward_scale = (1.0 / scaleSqrtN) * factor; // Apply (1/sqrtN) * (N/2)
                    break;
        ...
        // Converts standard complex array (size N/2+1) to vDSP split format for IRFFT.
        void pack_complex_to_split(const std::complex<T> *complex_input, void *split_output_void) const
        {
            ...
            if constexpr (std::is_same_v<T, float>) // SINGLE precision
            {
                ...
                split->imagp[0] = (N % 2 == 0 && Nc > 1) ? complex_input[Nc - 1].real() : 0.0f; // Nyquist component
            ...
            else // DOUBLE precision
            {
                ...
                split->realp[0] = complex_input[0].real(); // DC component
                split->imagp[0] = (N % 2 == 0 && Nc > 1) ? complex_input[Nc - 1].real() : 0.0; // Nyquist component
                for (size_t k = 1; k < N / 2; ++k) { // Components 1 to N/2 - 1
                    split->realp[k] = complex_input[k].real();
                    split->imagp[k] = complex_input[k].imag();
                }
        ...
        // Converts vDSP split format (from RFFT) to standard complex array (size N/2+1).
        ...
        {
            if (!setup_handle_c2c) throw std::runtime_error("C2C FFT plan not initialized.");

            // vDSP_DFT expects separate real and imaginary pointers for interleaved complex data.
            if constexpr (std::is_same_v<T, float>)
            {
        ...
        {
            if (!setup_handle_real) throw std::runtime_error("Real FFT plan not initialized.");
            if (domain != Domain::REAL || direction != Direction::FORWARD) {
                 throw std::runtime_error("execute_rfft called on incorrect plan type (must be REAL/FORWARD).");
            }
            ...
            if constexpr (std::is_same_v<T, float>)
            {
                ...
                DSPSplitComplex split_buffer = {const_cast<float *>(real_buffer.data()), const_cast<float *>(imag_buffer.data())};
                // Convert real input to split complex format
                vDSP_ctoz(reinterpret_cast<const DSPComplex *>(real_input), 2, &split_buffer, 1, length / 2);
                // Perform FFT
                vDSP_fft_zrip(setup_handle_real, &split_buffer, stride, get_log2n(length), kFFTDirection_Forward);
                ...
                if (std::abs(forward_scale - 1.0f) > std::numeric_limits<float>::epsilon()) {
                    vDSP_vsmul(reinterpret_cast<float *>(complex_output), 1, &forward_scale, reinterpret_cast<float *>(complex_output), 1, complex_length * 2);
                }
            }
            else // Double precision
            {
                DSPDoubleSplitComplex split_buffer = {const_cast<double *>(real_buffer.data()), const_cast<double *>(imag_buffer.data())};
                vDSP_ctozD(reinterpret_cast<const DSPDoubleComplex *>(real_input), 2, &split_buffer, 1, length / 2);
                vDSP_fft_zripD(setup_handle_real, &split_buffer, stride, get_log2n(length), kFFTDirection_Forward);
                unpack_split_to_complex(&split_buffer, complex_output);
                // Apply external scaling
        ...
        void execute_irfft_oop(const std::complex<T> *complex_input, T *real_output) const
        {
            if (!setup_handle_real) throw std::runtime_error("Real FFT plan not initialized.");
             if (domain != Domain::REAL || direction != Direction::INVERSE) {
                 throw std::runtime_error("execute_irfft called on incorrect plan type (must be REAL/INVERSE).");
            }
            ...
            if constexpr (std::is_same_v<T, float>)
            {
                ...
                // Perform inverse FFT
                vDSP_fft_zrip(setup_handle_real, &split_buffer, stride, get_log2n(length), kFFTDirection_Inverse);
                ...
                if (std::abs(backward_scale - 1.0f) > std::numeric_limits<float>::epsilon()) {
                    vDSP_vsmul(split_buffer.realp, 1, &backward_scale, split_buffer.realp, 1, length / 2);
                    vDSP_vsmul(split_buffer.imagp, 1, &backward_scale, split_buffer.imagp, 1, length / 2);
                }
                // Convert resulting split complex back to interleaved real output
                vDSP_ztoc(&split_buffer, 1, reinterpret_cast<DSPComplex *>(real_output), 2, length / 2);
            }
            else // Double precision
            {
                DSPDoubleSplitComplex split_buffer = {const_cast<double *>(real_buffer.data()), const_cast<double *>(imag_buffer.data())};
                pack_complex_to_split(complex_input, &split_buffer);
                vDSP_fft_zripD(setup_handle_real, &split_buffer, stride, get_log2n(length), kFFTDirection_Inverse);
                // Apply external scaling
                if (std::abs(backward_scale - 1.0) > std::numeric_limits<double>::epsilon()) {
                    vDSP_vsmulD(split_buffer.realp, 1, &backward_scale, split_buffer.realp, 1, length / 2);
                    vDSP_vsmulD(split_buffer.imagp, 1, &backward_scale, split_buffer.imagp, 1, length / 2);
                }
                vDSP_ztocD(&split_buffer, 1, reinterpret_cast<DSPDoubleComplex *>(real_output), 2, length / 2);
            }
        }

...
AccelerateOmniDSPImpl::AccelerateOmniDSPImpl() {
  // Perform any one-time setup specific to the Accelerate backend, if needed.
  // Often, Accelerate doesn't require explicit global initialization.
  std::cout << "Accelerate Backend Initialized." << std::endl;  // Debug message
}
...
AccelerateOmniDSPImpl::convolve(const std::vector<RealT<T>>& input,
                                const std::vector<RealT<T>>& kernel,
                                ConvolutionMode mode) const {
  // TODO: Implement 1D real convolution using Accelerate (e.g., vDSP_conv or
  // FFT based) Consider different modes (Full, Same, Valid)
...
AccelerateOmniDSPImpl::convolve(const std::vector<ComplexT<T>>& input,
                                const std::vector<ComplexT<T>>& kernel,
                                ConvolutionMode mode) const {
  // TODO: Implement 1D complex convolution using Accelerate (likely FFT based)
  std::cerr << "AccelerateOmniDSPImpl::convolve (complex) not yet implemented."
            << std::endl;
  return std::unexpected(Status::UnsupportedFeature);
}
...
template <typename T>
[[nodiscard]] OmniExpected<std::vector<RealT<T>>>
AccelerateOmniDSPImpl::correlate(const std::vector<RealT<T>>& input,
                                 const std::vector<RealT<T>>& kernel,
                                 ConvolutionMode mode) const {
  // TODO: Implement 1D real correlation using Accelerate (e.g., vDSP_conv or
  // FFT based, kernel not reversed)
  std::cerr << "AccelerateOmniDSPImpl::correlate (real) not yet implemented."
            << std::endl;
  return std::unexpected(Status::UnsupportedFeature);
}
...
template <typename T>
[[nodiscard]] OmniExpected<std::vector<ComplexT<T>>>
AccelerateOmniDSPImpl::correlate(const std::vector<ComplexT<T>>& input,
                                 const std::vector<ComplexT<T>>& kernel,
                                 ConvolutionMode mode) const {
  // TODO: Implement 1D complex correlation using Accelerate (likely FFT based,
  // conjugate kernel)
  std::cerr << "AccelerateOmniDSPImpl::correlate (complex) not yet implemented."
            << std::endl;
  return std::unexpected(Status::UnsupportedFeature);
}
...
template <typename T>
[[nodiscard]] OmniExpected<std::vector<ComplexT<T>>> AccelerateOmniDSPImpl::fft(
    const std::vector<ComplexT<T>>& input) const {
  // TODO: Implement one-off complex FFT using Accelerate (e.g., create
  // temporary plan)
  std::cerr << "AccelerateOmniDSPImpl::fft not yet implemented." << std::endl;
  return std::unexpected(Status::UnsupportedFeature);
}
...
AccelerateOmniDSPImpl::ifft(const std::vector<ComplexT<T>>& input) const {
  ...
  // temporary plan)
  std::cerr << "AccelerateOmniDSPImpl::ifft not yet implemented." << std::endl;
  return std::unexpected(Status::UnsupportedFeature);
}
...
AccelerateOmniDSPImpl::rfft(const std::vector<RealT<T>>& input) const {
  // TODO: Implement one-off real RFFT using Accelerate (e.g., create temporary
  // plan)
  std::cerr << "AccelerateOmniDSPImpl::rfft not yet implemented." << std::endl;
  return std::unexpected(Status::UnsupportedFeature);
}

template <typename T>
[[nodiscard]] OmniExpected<std::vector<RealT<T>>> AccelerateOmniDSPImpl::irfft(
    const std::vector<ComplexT<T>>& input, size_t output_length) const {
  // TODO: Implement one-off real IRFFT using Accelerate (e.g., create temporary
  // plan)
  std::cerr << "AccelerateOmniDSPImpl::irfft not yet implemented." << std::endl;
  return std::unexpected(Status::UnsupportedFeature);
}

// --- Window Generation (Implementations using vDSP) ---
// Helper to convert OmniDSP Status to vDSP status if needed, or handle errors
// directly
...
AccelerateOmniDSPImpl::blackman_window(size_t length) const {
  ...
  std::vector<RealT<T>> coeffs(length);
  if constexpr (std::is_same_v<T, float>) {
    vDSP_blkman_window(coeffs.data(), length, 0);  // 0 for standard Blackman
  } else {
...
AccelerateOmniDSPImpl::flattop_window(size_t length) const {
  // vDSP does not have a direct flat top window. Needs manual implementation.
  // Formula: a0 - a1*cos(2pi*n/N) + a2*cos(4pi*n/N) - a3*cos(6pi*n/N) +
  // a4*cos(8pi*n/N) Use standard coefficients (e.g., from SciPy) Can be
  // implemented using vDSP vector math (vramp, vsmul, cos, etc.)
...
AccelerateOmniDSPImpl::gaussian_window(size_t length, RealT<T> stddev) const {
  // vDSP does not have a direct Gaussian window. Needs manual implementation.
  // Formula: exp(-0.5 * ((n - (N-1)/2) / (sigma * (N-1)/2))^2)
  // Use vDSP vector math (vramp, vsadd, vsdiv, vsq, vsmul, vsadd, vexp)
...
AccelerateOmniDSPImpl::kaiser_window(size_t length, RealT<T> beta) const {
  ...
  // Formula involves Bessel function I0. Requires implementing I0 or using an
  // approximation.
...
AccelerateOmniDSPImpl::triangular_window(size_t length) const {
  // Similar to Bartlett, but often defined slightly differently (ends at zero
  // vs Bartlett ends near zero). Needs manual implementation using vDSP vector
  // math.
  std::cerr << "AccelerateOmniDSPImpl::triangular_window not yet implemented."
            << std::endl;
  return std::unexpected(Status::UnsupportedFeature);
}
...
AccelerateOmniDSPImpl::create_fft_plan(size_t length) const {
  try {
    ...
    // 2. Create the public Plan object using its private constructor, passing
    // the impl Need access to the private constructor (friend class OmniDSP in
    // FFTPlan) We achieve this by calling the constructor directly. Note: Using
    // std::make_unique requires a public constructor. We construct manually and
    // wrap.
...
AccelerateOmniDSPImpl::create_rfft_plan(size_t length) const {
  try {
    auto pimpl_backend = std::make_unique<AccelerateRFFTPlanImpl<T>>(length);
...
AccelerateOmniDSPImpl::create_cqt_plan(
  ...
  try {
    // Note: CQTPlan constructor is private/protected, but OmniDSP is a friend.
    // This factory method, being part of OmniDSPImpl (called by OmniDSP), can
    // facilitate. A clean way is to have the public OmniDSP::create_cqt_plan
    // call the private CQTPlan constructor directly, passing 'this' as the
    // owner. This backend implementation doesn't need to do much specific here,
    // unless backend choice affects CQT setup parameters.
    // Let's assume the main OmniDSP::create_cqt_plan forwards to this, and we
    // just construct.
...
AccelerateOmniDSPImpl::create_resample_plan(double input_rate,
                                            double output_rate,
...
AccelerateOmniDSPImpl::create_convolution_plan(const std::vector<T>& kernel,
                                               ConvolutionMode mode) const {
  try {
    auto pimpl_backend =
    ...
    std::cerr << "Error creating Accelerate ConvolutionPlanImpl: " << e.what()
              << std::endl;
    return std::unexpected(Status::BackendError);
  } catch (...) {
    return std::unexpected(Status::Failure);
...
AccelerateOmniDSPImpl::create_correlation_plan(const std::vector<T>& kernel,
                                               ConvolutionMode mode) const {
  try {
    auto pimpl_backend =
    ...
    std::cerr << "Error creating Accelerate CorrelationPlanImpl: " << e.what()
              << std::endl;
    return std::unexpected(Status::BackendError);
  } catch (...) {
    return std::unexpected(Status::Failure);
...
template OmniExpected<std::vector<float_c>> AccelerateOmniDSPImpl::convolve(
    ...
    ConvolutionMode) const;
template OmniExpected<std::vector<double_c>> AccelerateOmniDSPImpl::convolve(
...
template OmniExpected<std::vector<double>> AccelerateOmniDSPImpl::correlate(
    const std::vector<double>&, const std::vector<double>&,
    ConvolutionMode) const;
template OmniExpected<std::vector<float_c>> AccelerateOmniDSPImpl::correlate(
    const std::vector<float_c>&, const std::vector<float_c>&,
    ConvolutionMode) const;
template OmniExpected<std::vector<double_c>> AccelerateOmniDSPImpl::correlate(
    const std::vector<double_c>&, const std::vector<double_c>&,
    ConvolutionMode) const;
...
template OmniExpected<std::vector<double>>
    AccelerateOmniDSPImpl::blackman_window(size_t) const;
template OmniExpected<std::vector<float>> AccelerateOmniDSPImpl::flattop_window(
    size_t) const;
template OmniExpected<std::vector<double>>
...
template OmniExpected<std::vector<float>> AccelerateOmniDSPImpl::hamming_window(
    size_t) const;
template OmniExpected<std::vector<double>>
    AccelerateOmniDSPImpl::hamming_window(size_t) const;
template OmniExpected<std::vector<float>> AccelerateOmniDSPImpl::hann_window(
    size_t) const;
template OmniExpected<std::vector<double>> AccelerateOmniDSPImpl::hann_window(
    size_t) const;
template OmniExpected<std::vector<float>> AccelerateOmniDSPImpl::kaiser_window(
    size_t, float) const;
template OmniExpected<std::vector<double>> AccelerateOmniDSPImpl::kaiser_window(
    size_t, double) const;
template OmniExpected<std::vector<float>>
...
AccelerateOmniDSPImpl::create_cqt_plan(const OmniDSP*, float, float, float,
                                       int) const;
template OmniExpected<std::unique_ptr<CQTPlan<double>>>
AccelerateOmniDSPImpl::create_cqt_plan(const OmniDSP*, double, double, double,
                                       int) const;
...
AccelerateOmniDSPImpl::create_convolution_plan(const std::vector<float>&,
                                               ConvolutionMode) const;
template OmniExpected<std::unique_ptr<ConvolutionPlan<double>>>
AccelerateOmniDSPImpl::create_convolution_plan(const std::vector<double>&,
                                               ConvolutionMode) const;
template OmniExpected<std::unique_ptr<ConvolutionPlan<float_c>>>
AccelerateOmniDSPImpl::create_convolution_plan(const std::vector<float_c>&,
                                               ConvolutionMode) const;
template OmniExpected<std::unique_ptr<ConvolutionPlan<double_c>>>
AccelerateOmniDSPImpl::create_convolution_plan(const std::vector<double_c>&,
                                               ConvolutionMode) const;

template OmniExpected<std::unique_ptr<CorrelationPlan<float>>>
AccelerateOmniDSPImpl::create_correlation_plan(const std::vector<float>&,
                                               ConvolutionMode) const;
template OmniExpected<std::unique_ptr<CorrelationPlan<double>>>
AccelerateOmniDSPImpl::create_correlation_plan(const std::vector<double>&,
                                               ConvolutionMode) const;
template OmniExpected<std::unique_ptr<CorrelationPlan<float_c>>>
AccelerateOmniDSPImpl::create_correlation_plan(const std::vector<float_c>&,
                                               ConvolutionMode) const;
template OmniExpected<std::unique_ptr<CorrelationPlan<double_c>>>
AccelerateOmniDSPImpl::create_correlation_plan(const std::vector<double_c>&,
                                               ConvolutionMode) const;

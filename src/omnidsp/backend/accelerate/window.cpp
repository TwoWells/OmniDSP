...
// These definitions assume they are linked correctly to that class definition.
...
AccelerateOmniDSPImpl::bartlett_window(size_t length) const {
  ...
  if constexpr (std::is_same_v<T, float>) {
    vDSP_vsmul(n_vec.data(), 1, &scale, coeffs.data(), 1,
               length);  // coeffs = 2n / (N-1)
    vDSP_vsadd(coeffs.data(), 1, &minus_one, coeffs.data(), 1,
               length);  // coeffs = 2n / (N-1) - 1
    vDSP_vabs(coeffs.data(), 1, coeffs.data(), 1,
              length);  // coeffs = |2n / (N-1) - 1|
    vDSP_vneg(coeffs.data(), 1, coeffs.data(), 1, length);  // coeffs = -|...|
    vDSP_vsadd(coeffs.data(), 1, &one, coeffs.data(), 1,
               length);  // coeffs = 1 - |...|
  } else {
...
AccelerateOmniDSPImpl::flattop_window(size_t length) const {
  ...
  const RealT<T> factor =
      (N_minus_1 > 0) ? (static_cast<RealT<T>>(2.0 * M_PI) / N_minus_1) : 0.0;
  ...
  if constexpr (std::is_same_v<T, float>) {
    ...
    RealT<T> minus_a1 = -a1;
    RealT<T> minus_a3 = -a3;
    vDSP_vsma(term1.data(), 1, &minus_a1, coeffs.data(), 1, coeffs.data(), 1,
              length);  // -a1*t1
    vDSP_vsma(term2.data(), 1, &a2, coeffs.data(), 1, coeffs.data(), 1,
              length);  // +a2*t2
    vDSP_vsma(term3.data(), 1, &minus_a3, coeffs.data(), 1, coeffs.data(), 1,
              length);  // -a3*t3
    vDSP_vsma(term4.data(), 1, &a4, coeffs.data(), 1, coeffs.data(), 1,
              length);  // +a4*t4
    ...
    RealT<T> minus_a1 = -a1;
    RealT<T> minus_a3 = -a3;
    vDSP_vsmaD(term1.data(), 1, &minus_a1, coeffs.data(), 1, coeffs.data(), 1,
               length);
    vDSP_vsmaD(term2.data(), 1, &a2, coeffs.data(), 1, coeffs.data(), 1,
               length);
    vDSP_vsmaD(term3.data(), 1, &minus_a3, coeffs.data(), 1, coeffs.data(), 1,
               length);
    vDSP_vsmaD(term4.data(), 1, &a4, coeffs.data(), 1, coeffs.data(), 1,
               length);
  }
...
AccelerateOmniDSPImpl::gaussian_window(size_t length, RealT<T> stddev) const {
  ...
  // Handle potential division by zero if length=1 (center=0) -> sigma_term=0
  if (sigma_term == static_cast<RealT<T>>(0.0)) {
    // This case should be handled by length==1 check above, but defensive check
    coeffs[0] = 1.0;
    return coeffs;
  }
  ...
  RealT<T> start = 0.0;
  RealT<T> step = 1.0;
  int len_int = static_cast<int>(length);  // vv... functions take int* length
  if constexpr (std::is_same_v<T, float>) {
    vDSP_vramp(&start, &step, n_vec.data(), 1, length);
    // Calculate: exp( neg_half_inv_sigma_sq * (n - center)^2 )
    RealT<T> neg_center = -center;
    vDSP_vsadd(n_vec.data(), 1, &neg_center, coeffs.data(), 1,
               length);  // coeffs = n - center
    vDSP_vsq(coeffs.data(), 1, coeffs.data(), 1,
             length);  // coeffs = (n - center)^2
    vDSP_vsmul(coeffs.data(), 1, &neg_half_inv_sigma_sq, coeffs.data(), 1,
    ...
    vDSP_vsmulD(coeffs.data(), 1, &neg_half_inv_sigma_sq, coeffs.data(), 1,
                length);
...
AccelerateOmniDSPImpl::kaiser_window(size_t length, RealT<T> beta) const {
  ...
  if (bessel_i0_beta == static_cast<RealT<T>>(0.0)) {
    // Handle potential division by zero if I0(beta) is zero (shouldn't happen
    // for beta >= 0)
  ...
  if constexpr (std::is_same_v<T, float>) {
    vDSP_vsmul(n_vec.data(), 1, &scale, coeffs.data(), 1, length);  // 2n/(N-1)
    vDSP_vsadd(coeffs.data(), 1, &minus_one, coeffs.data(), 1,
               length);                                     // 2n/(N-1) - 1
...
AccelerateOmniDSPImpl::triangular_window(size_t length) const {
  ...
  if constexpr (std::is_same_v<T, float>) {
    if (L % 2 != 0) {  // Odd length (L=2k+1 -> N=k)
      ...
      vDSP_vramp(&start, &scale_up, coeffs.data(), 1,
                 (L + 1) / 2);  // Ramp 0 to 1
      vDSP_vramp(&one, &scale_down, coeffs.data() + (L + 1) / 2, 1,
                 L / 2);  // Ramp 1 to 0
      ...
      vDSP_vsmul(coeffs.data(), 1, &inv_L_half, coeffs.data(), 1,
                 L);                                     // |...| / (L/2)
...
// src/omnidsp/backend/accelerate/backend.cpp where the AccelerateOmniDSPImpl

...
namespace Backend
{
    ...
    std::vector<T> generate_hamming_window_impl(size_t length) {
        ...
        if constexpr (std::is_same_v<T, float>) {
            // vDSP_hamm_window_f(OutputVector, Length, Flags=0);
             vDSP_hamm_window_f(coeffs.data(), n_vDSP, 0); // Flags = 0 for standard Hamming
        } else { // double
    ...
    /**
     * @brief Generates Kaiser window coefficients (manual calculation, as vDSP lacks native support).
    ...
    std::vector<T> generate_kaiser_window_impl(size_t length, T beta) {
        ...
        for (size_t n = 0; n < length; ++n) {
            ...
            // Ensure argument to sqrt is non-negative (can happen at endpoints due to precision)
    ...
    /**
     * @brief Generates Flattop window coefficients (manual calculation, as vDSP lacks native support).
     */
    template <typename T>

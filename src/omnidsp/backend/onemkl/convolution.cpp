 ...
 * @brief Intel oneMKL backend implementation for OmniDSP convolution/correlation.
 *
...
namespace Backend
{
    ...
    // --- MKL VSL Helper ---
    // Checks the status code returned by MKL VSL functions and throws an error if it's not VSL_STATUS_OK.
     ...
     * Calculates the 'valid' part of the linear convolution/correlation.
     *
    ...
    std::vector<T> convolve1d_impl(const std::vector<T> &signal, const std::vector<T> &kernel, bool use_correlation)
    {
        ...
        if (signal.empty() || kernel.empty()) {
             // Return empty vector for empty inputs, consistent with public API expectation
             return {};
        }
        ...
        if (nz <= 0) {
            throw std::invalid_argument("MKL Conv/Corr ('valid' mode equivalent): signal length ("
        ...
        try {
            ...
            if constexpr (std::is_same_v<T, float>) { // SINGLE PRECISION
                if (call_mkl_convolution) {
                    ...
                    // Execute convolution
                    status = vslsConvExec(task, kernel.data(), &stride_kernel, signal.data(), &stride_signal, result.data(), &stride_result);
                    ...
                    // Execute correlation
                    status = vslsCorrExec(task, kernel.data(), &stride_kernel, signal.data(), &stride_signal, result.data(), &stride_result);
                 ...
                 if (call_mkl_convolution) {
                    ...
                    check_mkl_status(status, "MKL vslConvSetStart failed");
                    // Execute convolution
                    status = vsldConvExec(task, kernel.data(), &stride_kernel, signal.data(), &stride_signal, result.data(), &stride_result);
                    ...
                    check_mkl_status(status, "MKL vslCorrSetStart failed");
                    // Execute correlation
                    status = vsldCorrExec(task, kernel.data(), &stride_kernel, signal.data(), &stride_signal, result.data(), &stride_result);
            ...
            // --- Clean up VSL Task ---
            if (task) {
                status = vslConvDeleteTask(&task); // Use vslConvDeleteTask for both conv/corr tasks
            ...
            // Ensure task is deleted even if an exception occurs during checks/execution
            if (task) {

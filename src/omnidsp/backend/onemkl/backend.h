  ...
  namespace backend {
    ...
    class OneMKLRFFTPlanImpl final : public RFFTPlanImpl<T> {
     ...
     private:
      ...
      DFTI_DESCRIPTOR_HANDLE descriptor_handle_ = nullptr;
      MKL_LONG mkl_status_;
      // Store configuration values needed by DFTI ComputeForward/Backward for
      // real FFTs
      ...
      MKL_LONG output_strides_[1]
          = {0};  // For complex conjugate-even output/input
    };
    ...
    class OneMKLConvolutionPlanImpl final : public ConvolutionPlanImpl<T> {
     ...
     private:
      ...
      DFTI_DESCRIPTOR_HANDLE dfti_descriptor_
          = nullptr;  // Example if using DFTI
    ...
    class OneMKLCorrelationPlanImpl final : public CorrelationPlanImpl<T> {
     ...
     private:
      ...
      // Store either VSL Corr task handle or DFTI descriptor and FFT'd
      // (conjugated) template VSLTaskPtr vsl_task_ = nullptr; // Example

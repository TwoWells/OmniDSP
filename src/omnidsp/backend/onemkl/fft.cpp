...
namespace OmniDSP {
  namespace backend {
        ...
        : length_(length), mkl_status_(DFTI_NO_ERROR)
    {
      ...
      mkl_status_ = DftiSetValue(
          descriptor_handle_, DFTI_INPUT_STRIDES, input_strides_);
      if (mkl_status_ != DFTI_NO_ERROR) {
        DftiFreeDescriptor(&descriptor_handle_);
      ...
      // transform)
      mkl_status_
          = DftiSetValue(descriptor_handle_, DFTI_NUMBER_OF_TRANSFORMS, 1);
      if (mkl_status_ != DFTI_NO_ERROR) {
        DftiFreeDescriptor(&descriptor_handle_);
        throw std::runtime_error(
            "Failed to set DFTI_NUMBER_OF_TRANSFORMS (real).");
      }

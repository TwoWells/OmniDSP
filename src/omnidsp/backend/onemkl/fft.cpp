...
namespace OmniDSP
{
    ...
    // --- MKL DFTI Helper ---
    // Checks the status code returned by MKL DFTI functions and throws an error if it's not DFTI_NO_ERROR.
    ...
    // This struct holds the MKL-specific data (DFTI descriptor) and logic for FFTPlan.
    template <typename T>
    struct FFTPlanImpl
    {
        // --- Members ---
        ...
        FFTPlanImpl(size_t len, Precision prec, Direction dir, Domain dom, NormMode norm)
        {
            ...
            if (scaleSqrtN == static_cast<T>(0.0) && norm_mode == NormMode::ORTHO)
                throw std::runtime_error("ScaleSqrtN is zero for ORTHO (length must be > 0)");
            ...
            // In-place execution is handled by simulating with a temporary buffer if needed.
        ...
        ~FFTPlanImpl()
        {
            if (handle)
            {
                // DftiFreeDescriptor returns a status, but we usually ignore it in destructors.
        ...
        {
            if (!handle) throw std::runtime_error("MKL FFTPlan handle is null (execute_c2c_oop).");
            MKL_LONG status;
        ...
        {
             if (!handle) throw std::runtime_error("MKL FFTPlan handle is null (execute_c2c_ip).");
             MKL_LONG status;
             // MKL DFTI doesn't guarantee true in-place for all configurations.
             // Simulate using the out-of-place version with a temporary buffer for safety.
        ...
        {
             if (!handle) throw std::runtime_error("MKL FFTPlan handle is null (execute_rfft_oop).");
        ...
        {
             if (!handle) throw std::runtime_error("MKL FFTPlan handle is null (execute_irfft_oop).");
    ...
    // These definitions link the public FFTPlan class methods to the PIMPL implementation.
    ...
    // These ensure the public FFTPlan class itself is instantiated and exported correctly.
    // OMNIDSP_EXPORT is needed here if building a shared library/DLL.

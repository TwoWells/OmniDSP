...
inline Status ipp_status_to_omnidsp_status(IppStatus status) {
  ...
  return Status::BackendError;  // Generic backend error for other IPP issues
}

//--------------------------------------------------------------------------
// OneMKLOmniDSPImpl - Window Generation Method Implementations using IPP/VML
//--------------------------------------------------------------------------
 ...
 * @details IPP does not have a direct Flat Top function, so MKL VML is used for
 * calculation.
...
OneMKLOmniDSPImpl::flattop_window(size_t length) const {
  ...
  std::vector<RealT<T>> arg1(length), arg2(length), arg3(length), arg4(length);
  std::vector<RealT<T>> cos1(length), cos2(length), cos3(length), cos4(length);
  RealT<T> f1 = factor, f2 = factor * 2.0, f3 = factor * 3.0, f4 = factor * 4.0;
  ...
  if constexpr (std::is_same_v<T, float>) {
    ...
    vsAdd(n_mkl, coeffs.data(), arg2.data(), coeffs.data());  // coeffs += a2*c2
    vsSub(n_mkl, coeffs.data(), arg3.data(), coeffs.data());  // coeffs -= a3*c3
    vsAdd(n_mkl, coeffs.data(), arg4.data(), coeffs.data());  // coeffs += a4*c4
  } else {                                                    // Double
 ...
 * @details IPP does not have a direct Gaussian function, so MKL VML is used.
...
OneMKLOmniDSPImpl::gaussian_window(size_t length, RealT<T> stddev) const {
  ...
  if (sigma_term ==
      static_cast<RealT<T>>(0.0)) {  // Avoid division by zero if length=1
    coeffs[0] = 1.0;
    return coeffs;
  }
  ...
  if constexpr (std::is_same_v<T, float>) {
    vsAdd(n_mkl, n_vec.data(), &neg_center, temp.data());  // temp = n - center
    vsSqr(n_mkl, temp.data(), temp.data());  // temp = (n - center)^2
    vsMul(n_mkl, temp.data(), &neg_half_inv_sigma_sq,
 ...
 * @details Converts the standard beta parameter to the alpha parameter used by
 * IPP using the relationship alpha = beta / pi.
...
OneMKLOmniDSPImpl::kaiser_window(size_t length, RealT<T> beta) const {
  ...
  if (status != ippStsNoErr) {
    ...
    std::cerr << "IPP Kaiser window generation failed. Beta/Alpha (" << alpha
              << ") might be too large or length (" << length
...
OneMKLOmniDSPImpl::rectangular_window(size_t length) const {
  ...
  if (status != ippStsNoErr) {
    // Fallback or error
    std::cerr
        << "IPP Set failed for rectangular window, falling back to std::vector."
        << std::endl;
...
OneMKLOmniDSPImpl::triangular_window(size_t length) const {
  // IPP doesn't have a distinct triangular window. The Bartlett window is often
  // considered equivalent or very similar (depending on endpoint definitions).
  // We reuse the Bartlett implementation here.

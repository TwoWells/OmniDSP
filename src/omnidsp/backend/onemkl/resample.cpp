 ...
 * Corrected argument count for ippsResamplePolyphaseFixedGetSize_32f to 7 arguments.
...
namespace Backend
{
    ...
    /**
     * @brief MKL/IPP backend implementation for combined FIR filtering and downsampling.
     * Uses Intel IPP polyphase resampling (ippsResamplePolyphaseFixed) for FLOAT only.
     * Double precision (_64f) version is currently unsupported due to apparent lack of
     * IPP function availability/compilation issues in tested versions.
     *
     * @tparam T float or double.
     * @param signal The input signal vector.
     * @param kernel The FIR filter coefficients vector (used by IPP internally for length hint).
     * @param factor The integer downsampling factor (M > 0).
     * @return std::vector<T> The filtered and downsampled signal.
     * @throws std::invalid_argument If inputs are invalid (empty, factor <= 0, kernel empty/invalid).
     * @throws std::runtime_error If an IPP error occurs, or if called with double precision.
     */
    template <typename T>
    std::vector<T> filter_and_downsample_impl(const std::vector<T> &signal, const std::vector<T> &kernel, int factor)
    {
        ...
        else if constexpr (std::is_same_v<T, float>)
        {
            ...
            if (kernel.empty()) throw std::invalid_argument("Kernel cannot be empty (used for filter length).");

            int inLen = static_cast<int>(signal.size());
            int filterLenHint = static_cast<int>(kernel.size()); // Use provided kernel size as length hint
            if (filterLenHint <= 0) throw std::invalid_argument("IPP Resampling filter length hint must be positive.");
            ...
            try {
                ...
                status = ippsResamplePolyphaseFixedGetSize_32f(inRate, outRate, filterLenHint, &specSize, &filterLenUsed, &filterHeight, hint);
                ...
                status = ippsResamplePolyphaseFixedInit_32f(inRate, outRate, filterLenHint, rolloff, alpha, pSpec, hint);
                ...
                // Use filterLenUsed (returned by GetSize) for potentially more accurate margin? Or stick with hint?
                // Let's stick with filterLenHint for margin consistency for now.
                size_t maxOutLen = static_cast<size_t>(std::ceil(static_cast<double>(inLen) * outRate / inRate)) + filterLenHint;
                ...
                // 5. Perform Resampling (Filter + Downsample)
                 ...
                 // The execution function signature also needs verification, assuming 7 args based on common patterns
                 // IPPAPI(IppStatus, ippsResamplePolyphaseFixed_32f,(const Ipp32f* pSrc, int srcLen, Ipp32f* pDst, Ipp32f norm, double* pTime, int* pDstLen, IppsResamplePolyphaseFixed_32f* pSpec))
                 ...
                 check_ipp_status(status, "IPP ippsResamplePolyphaseFixed_32f execution failed");
                ...
                // 7. Free IPP buffers
                if (pSpecBuffer) { ippsFree(pSpecBuffer); pSpecBuffer = nullptr; } // Free the original buffer
    ...
    // template std::vector<double> filter_and_downsample_impl<double>(const std::vector<double> &, const std::vector<double> &, int); // DO NOT INSTANTIATE DOUBLE

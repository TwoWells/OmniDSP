 ...
 * @brief Implementation of public resampling functions, dispatching to backends.
 */
 ...
 namespace OmniDSP
 {
     ...
     /**
      * @brief Performs combined FIR filtering and downsampling by calling the backend implementation.
      */
     template <typename T>
     ...
     template std::vector<float> filter_and_downsample<float>(const std::vector<float> &signal, const std::vector<float> &kernel, int factor);
     template std::vector<double> filter_and_downsample<double>(const std::vector<double> &signal, const std::vector<double> &kernel, int factor);

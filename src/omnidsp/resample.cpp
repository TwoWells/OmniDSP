 ...
 * @param output A span representing the output buffer for the resampled signal.
...
[[nodiscard]] Status ResamplePlan<T>::execute(std::span<const T> input,
  ...
  // points to can have its non-const execute method called. If the Impl execute
  // needs to modify the Impl state, the Impl members must be mutable. Let's
  // cast away constness here, assuming the Impl execute modifies mutable state.
  // A potentially cleaner design might make the public execute non-const if
  // state modification is expected. However, following the pattern of FFTPlan
  // etc., we keep public execute const.

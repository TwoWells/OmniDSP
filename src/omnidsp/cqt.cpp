 ...
 */
 ...
 namespace OmniDSP
 {
           ...
           sparsity_threshold_(sparsity_threshold)
     {
         ...
         if (!window_function_) throw std::invalid_argument("A valid window function must be provided.");
         if (hop_length_ == 0) throw std::invalid_argument("Hop length must be positive.");
         if (fir_filter_order <= 0) throw std::invalid_argument("FIR filter order must be positive.");
         if (fir_filter_order % 2 == 0) throw std::invalid_argument("FIR filter order must be odd for windowed-sinc design.");
         fir_filter_order_ = fir_filter_order;
         if (sparsity_threshold_ < 0) throw std::invalid_argument("Sparsity threshold cannot be negative.");
         ...
         if (num_octaves_ > 0)
         {
             ...
             for (int octave_idx = 0; octave_idx < num_octaves_; ++octave_idx)
             {
                 ...
                 try {
                     octave_signal_fft_plans_[octave_idx] = std::make_unique<FFTPlan<T>>(fft_len_oct, prec, Direction::FORWARD, Domain::REAL, NormMode::BACKWARD);
                     octave_kernel_fft_plans_[octave_idx] = std::make_unique<FFTPlan<T>>(fft_len_oct, prec, Direction::FORWARD, Domain::COMPLEX, NormMode::BACKWARD);
                 ...
                 for (int k = 0; k < bins_in_this_octave; ++k) {
                     ...
                     if (Nk == 0 || Nk > fft_len_oct) {
                         #ifdef OMNIDSP_ENABLE_CQT_WARNINGS
                         std::cerr << "Warning: Clamping Nk (" << Nk << ") to FFT length (" << fft_len_oct << ") for octave " << octave_idx << ", bin " << k << std::endl;
                         #endif
     ...
     {
         if (num_bins_ == 0 || num_octaves_ == 0) { output.clear(); return; }
         if (input.empty()) { output.assign(num_bins_, {}); return; }
         ...
         for (int octave_idx = num_octaves_ - 1; octave_idx >= 0; --octave_idx) {
             ...
             std::vector<std::vector<std::complex<T>>> octave_cqt_coeffs = calculateSingleOctaveCQT(current_y, current_sr, current_hop, octave_idx);
         ...
         for (int octave_idx = 0; octave_idx < num_octaves_; ++octave_idx) {
             ...
             if (octave_data.empty() || (bins_in_this_octave > 0 && (octave_data[0].empty() && num_frames > 0))) {
                 current_bin_offset += bins_in_this_octave;
                 continue;
             }

             size_t bins_in_oct_data = octave_data.size();
             size_t frames_in_this_octave = (bins_in_oct_data > 0 && !octave_data[0].empty()) ? octave_data[0].size() : 0;

             if (static_cast<size_t>(bins_in_this_octave) != bins_in_oct_data) {
                 #ifdef OMNIDSP_ENABLE_CQT_WARNINGS
                 std::cerr << "Warning: CQT bin count mismatch for octave " << octave_idx << " (expected " << bins_in_this_octave << ", got " << bins_in_oct_data << "). Using actual data size for offset." << std::endl;
                 #endif
     ...
     // --- Getter Definitions ---
     template <typename T> size_t CQTPlan<T>::getNumBins() const { return num_bins_; }
     template <typename T> double CQTPlan<T>::getSampleRate() const { return sample_rate_; }
     template <typename T> size_t CQTPlan<T>::getHopLength() const { return hop_length_; }
     template <typename T> double CQTPlan<T>::getLowestFrequency() const { return lowest_freq_; }
     template <typename T> double CQTPlan<T>::getHighestFrequency() const { return highest_freq_; }
     template <typename T> int CQTPlan<T>::getBinsPerOctave() const { return bins_per_octave_; }
     template <typename T> int CQTPlan<T>::getNumOctaves() const { return num_octaves_; }
     template <typename T> T CQTPlan<T>::getSparsityThreshold() const { return sparsity_threshold_; }
     template <typename T> int CQTPlan<T>::getFirFilterOrder() const { return fir_filter_order_; }
     ...
     /**
      * @brief Calculates FIR filter coefficients for anti-aliasing using windowed-sinc.
      * Uses backend-provided window coefficients (defaulting to Hann).
      */
     template <typename T>
     std::vector<T> CQTPlan<T>::calculateFirCoefficients(double current_sample_rate, int N) const
     {
         if (N <= 0) throw std::invalid_argument("FIR filter order N must be positive.");
         if (N % 2 == 0) {
             #ifdef OMNIDSP_ENABLE_CQT_WARNINGS
             std::cerr << "Warning: FIR filter order N (" << N << ") should preferably be odd." << std::endl;
             #endif
         }
         if (current_sample_rate <= 0.0) throw std::invalid_argument("Current sample rate must be positive.");
         ...
         try {
             ...
             throw std::runtime_error(std::string("Backend window generation failed in calculateFirCoefficients: ") + e.what());
         }
         ...
         for (int n = 0; n < N; ++n) {
             ...
             h_coeffs[n] = ideal_response * window_coeffs[n]; // Apply the generated window
             sum_h += h_coeffs[n];
         }
         ...
         if (std::abs(sum_h) > std::numeric_limits<T>::epsilon() * N) {
             ...
             #ifdef OMNIDSP_ENABLE_CQT_WARNINGS
             std::cerr << "Warning: FIR filter coefficient sum is close to zero. Normalization skipped." << std::endl;
             #endif
             if (N > 0) {
      ...
      * Calls the backend implementation. Throws error for double precision if backend doesn't support it.
      */
     template <typename T>
     std::vector<T> CQTPlan<T>::filterAndDownsampleBy2(
         const std::vector<T> &signal, double current_sample_rate) const
     {
         // --- Check for double precision - KEEP this check as it reflects backend limitation ---
         if constexpr (std::is_same_v<T, double>) {
             // Check if the MKL backend is active, as only it currently has this known limitation
             #if defined(USE_ONEMKL) && !defined(USE_ACCELERATE) // Be more specific
                 throw std::runtime_error("CQTPlan internal downsampling is not supported for double precision with the current MKL/IPP backend configuration.");
             #endif
             // If Accelerate or Stub backend, proceed (Accelerate supports double, Stub throws anyway)
         }
         // --- End Check ---

 ...
 * function adaptation and 2D output conversion.
 */
 ...
 */
template <typename T>
py::array_t<T> vector_vector_to_numpy_2d(const std::vector<std::vector<T>> &vec)
{
    if (vec.empty())
    {
    ...
    for (size_t i = 0; i < rows; ++i)
    {
        ...
        if (vec[i].size() != cols)
        {
            throw std::runtime_error("Cannot convert vector<vector> to NumPy array: inner vectors have inconsistent sizes.");
        }
        // Copy data from the current inner vector (vec[i]) to the corresponding row in the NumPy array.
        // The destination pointer (ptr + i * cols) calculates the start of the i-th row in the NumPy buffer.
...
PYBIND11_MODULE(omnidsp_py, m)
{
    ...
    // --- Bind Enums ---
    // Expose C++ enums to Python. export_values() makes enum members accessible directly.
    ...
    // --- Bind FFTPlan Class ---
    // Expose the FFTPlan class template for float and double precision separately.
    // We bind the constructor and getter methods. Execution is typically done via convenience functions.
    ...
    py::class_<FFTPlanFloat>(m, "FFTPlanFloat", R"pbdoc(Manages a pre-calculated plan for efficient FFT execution (float precision).)pbdoc")
        .def(py::init<size_t, OmniDSP::Precision, OmniDSP::Direction, OmniDSP::Domain, OmniDSP::NormMode>(),
             ...
             py::arg("norm") = OmniDSP::NormMode::BACKWARD) // Default normalization mode
        // Bind getter methods
    ...
    const int DEFAULT_FIR_ORDER = 101; // Matches DEFAULT_RECURSIVE_FIR_ORDER in cqt.h

    py::class_<CQTPlanFloat>(m, "CQTPlanFloat", R"pbdoc(Manages a pre-calculated plan for efficient recursive CQT execution (float precision).)pbdoc")
        // Bind constructor using a lambda function to handle Python callable adaptation
        .def(py::init([](double sample_rate, size_t hop_length, double lowest_freq, double highest_freq, int bins_per_octave,
                         py::object py_window_func, float sparsity_threshold, int fir_filter_order) { // Accept Python object and new args
                 ...
                 // Create a C++ std::function wrapper that calls the Python function.
                 // This lambda captures the Python function object.
                 CppWindowFuncFloat cpp_func =
                     [py_func = std::move(py_window_func)](size_t length) -> std::vector<float>
                 {
                     // The C++ CQTPlan calls this lambda with the required window length.
                     // We need to call the *original* Python function.
                     // Since the Python function likely expects a NumPy array (e.g., like np.hanning),
                     // we create a dummy NumPy array of the correct size and dtype to pass to it.
                     py::array_t<float> dummy_arg(length);   // Dummy argument with correct size
                     py::object result = py_func(dummy_arg); // Call the Python function

                     // Convert the Python function's result (expected to be a NumPy array or compatible)
                     // back to a std::vector<float> for C++.
                     try
                     {
                         py::array_t<float> result_arr = py::cast<py::array_t<float>>(result);
                         py::buffer_info info = result_arr.request();
                         if (info.ndim != 1)
                             throw std::runtime_error("Window function must return a 1D array/list");
                         ...
                         return std::vector<float>(ptr, ptr + info.shape[0]); // Create vector from buffer
                     }
                 ...
                 // Construct the CQTPlanFloat instance, passing the C++ lambda wrapper and other arguments.
                 return std::make_unique<CQTPlanFloat>(sample_rate, hop_length, lowest_freq, highest_freq, bins_per_octave,
             ...
             py::arg("sample_rate"), py::arg("hop_length"), py::arg("lowest_freq"), py::arg("highest_freq"), py::arg("bins_per_octave"), py::arg("window_function"), py::arg_v("sparsity_threshold", DEFAULT_SPARSITY_FLOAT, "Sparsity threshold for kernels (default: 1e-5)"), // Optional arg
             py::arg_v("fir_filter_order", DEFAULT_FIR_ORDER, "Order of FIR anti-aliasing filter (default: 101)")                                                                                                                                                               // Optional arg
             )
        ...
        .def("execute", [](const CQTPlanFloat &plan, py::array_t<float, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<std::complex<float>>
             {
                  ...
                  return vector_vector_to_numpy_2d(output_vec); }, py::arg("input"), R"pbdoc(Executes float CQT transform. Returns a 2D NumPy array (bins x frames).)pbdoc")
        // Bind getters (including newly added ones)
        .def("getNumBins", &CQTPlanFloat::getNumBins, "Gets the total number of CQT frequency bins.")
        .def("getSampleRate", &CQTPlanFloat::getSampleRate, "Gets the initial sample rate used (Hz).")
        .def("getHopLength", &CQTPlanFloat::getHopLength, "Gets the initial hop length used (samples).")
        ...
        .def("getNumOctaves", &CQTPlanFloat::getNumOctaves, "Gets the number of octaves processed.")
        .def("getSparsityThreshold", &CQTPlanFloat::getSparsityThreshold, "Gets the sparsity threshold used for kernels.")
        .def("getFirFilterOrder", &CQTPlanFloat::getFirFilterOrder, "Gets the FIR anti-aliasing filter order.");
    // .def("getOctaveFFTLengths", &CQTPlanFloat::getOctaveFFTLengths, "Gets FFT lengths per octave.") // Example if getter was added

    py::class_<CQTPlanDouble>(m, "CQTPlanDouble", R"pbdoc(Manages a pre-calculated plan for efficient recursive CQT execution (double precision).)pbdoc")
        // Bind constructor using a lambda function - Updated for new arguments
        .def(py::init([](double sample_rate, size_t hop_length, double lowest_freq, double highest_freq, int bins_per_octave,
                         py::object py_window_func, double sparsity_threshold, int fir_filter_order) { // Accept Python object and new args
                 if (!py::isinstance<py::function>(py_window_func))
                 {
                     throw py::type_error("window_function must be a Python callable");
                 }
                 ...
                 return std::make_unique<CQTPlanDouble>(sample_rate, hop_length, lowest_freq, highest_freq, bins_per_octave,
                                                        cpp_func, // Pass the C++ lambda wrapper
                                                        sparsity_threshold, fir_filter_order);
             }),
             "Constructs double CQT plan.", // Docstring for constructor
             // Define arguments for Python users, including new ones with defaults
             py::arg("sample_rate"), py::arg("hop_length"), py::arg("lowest_freq"), py::arg("highest_freq"), py::arg("bins_per_octave"), py::arg("window_function"), py::arg_v("sparsity_threshold", DEFAULT_SPARSITY_DOUBLE, "Sparsity threshold for kernels (default: 1e-5)"), // Optional arg
             py::arg_v("fir_filter_order", DEFAULT_FIR_ORDER, "Order of FIR anti-aliasing filter (default: 101)")                                                                                                                                                                // Optional arg
             )
        // Bind execute method - Updated for 2D output
        .def("execute", [](const CQTPlanDouble &plan, py::array_t<double, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<std::complex<double>>
             {
                  ...
                  return vector_vector_to_numpy_2d(output_vec); }, py::arg("input"), R"pbdoc(Executes double CQT transform. Returns a 2D NumPy array (bins x frames).)pbdoc")
        // Bind getters (including newly added ones)
        .def("getNumBins", &CQTPlanDouble::getNumBins, "Gets the total number of CQT frequency bins.")
        .def("getSampleRate", &CQTPlanDouble::getSampleRate, "Gets the initial sample rate used (Hz).")
        .def("getHopLength", &CQTPlanDouble::getHopLength, "Gets the initial hop length used (samples).")
        ...
        .def("getNumOctaves", &CQTPlanDouble::getNumOctaves, "Gets the number of octaves processed.")
        .def("getSparsityThreshold", &CQTPlanDouble::getSparsityThreshold, "Gets the sparsity threshold used for kernels.")
        .def("getFirFilterOrder", &CQTPlanDouble::getFirFilterOrder, "Gets the FIR anti-aliasing filter order.");
    // .def("getOctaveFFTLengths", &CQTPlanDouble::getOctaveFFTLengths, "Gets FFT lengths per octave.") // Example if getter was added

    // --- Bind Window Functions ---
    // Bind the static methods of the C++ Window class.
    // Use pybind11's function overloading feature to expose a single Python function
    // (e.g., Window.hann) that calls the correct C++ template specialization based on input type.
    py::class_<OmniDSP::Window>(m, "Window", "Provides common window functions for signal processing.")
        ...
        .def_static("hann", [](py::array_t<float, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<float>
             ...
             return vector_to_numpy_1d(result_vec); }, py::arg("input"), "Applies Hann window (float32 input/output).") // Overload 1: float
        // Bind double version
        .def_static("hann", [](py::array_t<double, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<double>
             ...
             // Convert result back to NumPy array
             return vector_to_numpy_1d(result_vec); }, py::arg("input"), "Applies Hann window (float64 input/output).") // Overload 2: double
        ...
        .def_static("hamming", [](py::array_t<float, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<float>
                    {
             std::vector<float> input_vec(np_input.data(), np_input.data() + np_input.size());
             return vector_to_numpy_1d(OmniDSP::Window::hamming(input_vec)); }, py::arg("input"), "Applies Hamming window (float32).")
        .def_static("hamming", [](py::array_t<double, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<double>
                    {
             std::vector<double> input_vec(np_input.data(), np_input.data() + np_input.size());
             return vector_to_numpy_1d(OmniDSP::Window::hamming(input_vec)); }, py::arg("input"), "Applies Hamming window (float64).")
        ...
        .def_static("kaiser", [](py::array_t<float, py::array::c_style | py::array::forcecast> np_input, float beta) -> py::array_t<float>
             ...
             std::vector<float> input_vec(np_input.data(), np_input.data() + np_input.size());
             // Call C++ kaiser<float>
             return vector_to_numpy_1d(OmniDSP::Window::kaiser(input_vec, beta)); }, py::arg("input"), py::arg("beta"), "Applies Kaiser window (float32).")
        .def_static("kaiser", [](py::array_t<double, py::array::c_style | py::array::forcecast> np_input, double beta) -> py::array_t<double>
             ...
             return vector_to_numpy_1d(OmniDSP::Window::kaiser(input_vec, beta)); }, py::arg("input"), py::arg("beta"), "Applies Kaiser window (float64).")

        // --- Flattop Window ---
        .def_static("flattop", [](py::array_t<float, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<float>
                    {
             std::vector<float> input_vec(np_input.data(), np_input.data() + np_input.size());
             return vector_to_numpy_1d(OmniDSP::Window::flattop(input_vec)); }, py::arg("input"), "Applies Flat-top window (float32).")
        .def_static("flattop", [](py::array_t<double, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<double>
                    {
             std::vector<double> input_vec(np_input.data(), np_input.data() + np_input.size());
             return vector_to_numpy_1d(OmniDSP::Window::flattop(input_vec)); }, py::arg("input"), "Applies Flat-top window (float64).");

    // --- Bind Convenience Functions ---
    // Bind the C++ convenience functions (fft, ifft, rfft, irfft).
    // Use pybind11 function overloading to handle float/double versions based on NumPy input type.
    ...
    m.def("fft", [](py::array_t<std::complex<float>, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<std::complex<float>>
         ...
         std::vector<std::complex<float>> output_vec; // Output vector to be filled by C++ function.
         OmniDSP::fft(input_vec, output_vec); // Calls C++ OmniDSP::fft<float>
         // Convert result back to NumPy array.
         return vector_to_numpy_1d(output_vec); }, py::arg("input"), "Performs C2C forward FFT (complex64 input/output, NormMode.BACKWARD)."); // Overload 1: float
    ...
    m.def("fft", [](py::array_t<std::complex<double>, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<std::complex<double>>
         ...
         // Convert result back to NumPy array.
         return vector_to_numpy_1d(output_vec); }, py::arg("input"), "Performs C2C forward FFT (complex128 input/output, NormMode.BACKWARD)."); // Overload 2: double
    ...
    m.def("ifft", [](py::array_t<std::complex<float>, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<std::complex<float>>
         ...
         return vector_to_numpy_1d(output_vec); }, py::arg("input"), "Performs C2C inverse FFT (complex64 input/output, NormMode.BACKWARD).");
    ...
    m.def("ifft", [](py::array_t<std::complex<double>, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<std::complex<double>>
         ...
         return vector_to_numpy_1d(output_vec); }, py::arg("input"), "Performs C2C inverse FFT (complex128 input/output, NormMode.BACKWARD).");
    ...
    m.def("rfft", [](py::array_t<float, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<std::complex<float>>
         ...
         return vector_to_numpy_1d(output_vec); }, py::arg("real_input"), "Performs R2C forward FFT (float32 input -> complex64 output, NormMode.BACKWARD). Returns N/2+1 complex points.");
    ...
    m.def("rfft", [](py::array_t<double, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<std::complex<double>>
         ...
         return vector_to_numpy_1d(output_vec); }, py::arg("real_input"), "Performs R2C forward FFT (float64 input -> complex128 output, NormMode.BACKWARD). Returns N/2+1 complex points.");
    ...
    m.def("irfft", [](py::array_t<std::complex<float>, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<float>
         ...
         return vector_to_numpy_1d(output_vec); }, py::arg("complex_input"), "Performs C2R inverse FFT (complex64 input -> float32 output, NormMode.BACKWARD). Input must have Hermitian symmetry.");
    ...
    m.def("irfft", [](py::array_t<std::complex<double>, py::array::c_style | py::array::forcecast> np_input) -> py::array_t<double>
         ...
         return vector_to_numpy_1d(output_vec); }, py::arg("complex_input"), "Performs C2R inverse FFT (complex128 input -> float64 output, NormMode.BACKWARD). Input must have Hermitian symmetry.");

    // --- Version Info ---
    // Embed version information into the module if defined during compilation (e.g., via CMake -DVERSION_INFO=...)
#ifdef VERSION_INFO

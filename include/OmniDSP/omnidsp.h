 ...
 * backends like Intel oneMKL and Apple Accelerate. Includes export macros
 * for Windows DLL compatibility.
 *
 * @version 1.0.1 // Version bump for header fix
 * @date 2025-04-13 // Or current date
 */
 ...
 // This header is generated by CMake's GenerateExportHeader module
 // and defines OMNIDSP_EXPORT as __declspec(dllexport) or __declspec(dllimport)
 // or empty depending on the build context (building DLL vs using DLL).
 ...
 #include <cmath>       // For std::cyl_bessel_i (though maybe only needed by windows.h?)
 ...
 namespace OmniDSP
 {
      ...
      *
      * This class is move-only (not copyable). Marked with OMNIDSP_EXPORT for DLL usage.
      *
      * @tparam T The floating-point type (float or double).
      */
     template <typename T>
     class OMNIDSP_EXPORT FFTPlan
     { // <--- OMNIDSP_EXPORT macro added here
     ...
     public:
         ...
         // --- MODIFICATION START ---
         /** @brief Move constructor. Explicitly defaulted in header. */
         FFTPlan(FFTPlan &&) noexcept = default;
         /** @brief Move assignment operator. Explicitly defaulted in header. */
     ...
     // Template functions fully defined in headers or instantiated only in the DLL's
     // cpp files generally do not need explicit export macros on the declaration here,
     // but the extern template declarations below *do* need it.
     ...
     extern template class OMNIDSP_EXPORT FFTPlan<float>;  // Add export macro here too
     extern template class OMNIDSP_EXPORT FFTPlan<double>; // Add export macro here too
 
     // Add OMNIDSP_EXPORT to extern template function declarations
     // This declares that these specific instantiations exist elsewhere (in the DLL)
     // and should be imported when linking against the DLL.

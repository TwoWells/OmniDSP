 ...
 */
 ...
 namespace OmniDSP
 {
      ...
      * @param kernel The FIR filter coefficients vector. Note: Some backends (like IPP FixedPolyphase)
      * might use the kernel's length to design an internal filter rather than using
      ...
      * @throws std::invalid_argument If inputs are invalid (e.g., empty signal/kernel, factor <= 0).
      */
     template <typename T>
     ...
     extern template std::vector<float> filter_and_downsample<float>(const std::vector<float> &signal, const std::vector<float> &kernel, int factor);
     extern template std::vector<double> filter_and_downsample<double>(const std::vector<double> &signal, const std::vector<double> &kernel, int factor);
     /** @endcond */

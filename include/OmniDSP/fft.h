 ...
 * including the FFTPlan class and convenience functions. It relies on core enums
 * being defined elsewhere (e.g., in omnidsp.h).
 *
 ...
 */
 ...
 // Note: This header assumes the core OmniDSP Enums (Direction, Precision, Domain, NormMode)
 //       are defined before this header is included, typically by including <OmniDSP/omnidsp.h>.
 ...
 namespace OmniDSP
 {
     ...
     extern template OMNIDSP_EXPORT void fft<float>(const std::vector<std::complex<float>> &, std::vector<std::complex<float>> &);
     extern template OMNIDSP_EXPORT void fft<double>(const std::vector<std::complex<double>> &, std::vector<std::complex<double>> &);
     extern template OMNIDSP_EXPORT void ifft<float>(const std::vector<std::complex<float>> &, std::vector<std::complex<float>> &);
     extern template OMNIDSP_EXPORT void ifft<double>(const std::vector<std::complex<double>> &, std::vector<std::complex<double>> &);
     extern template OMNIDSP_EXPORT void fft_inplace<float>(std::vector<std::complex<float>> &);
     extern template OMNIDSP_EXPORT void fft_inplace<double>(std::vector<std::complex<double>> &);
     extern template OMNIDSP_EXPORT void ifft_inplace<float>(std::vector<std::complex<float>> &);
     extern template OMNIDSP_EXPORT void ifft_inplace<double>(std::vector<std::complex<double>> &);
     extern template OMNIDSP_EXPORT void rfft<float>(const std::vector<float> &, std::vector<std::complex<float>> &);
     extern template OMNIDSP_EXPORT void rfft<double>(const std::vector<double> &, std::vector<std::complex<double>> &);
     extern template OMNIDSP_EXPORT void irfft<float>(const std::vector<std::complex<float>> &, std::vector<float> &);
     extern template OMNIDSP_EXPORT void irfft<double>(const std::vector<std::complex<double>> &, std::vector<double> &);
     /** @endcond */

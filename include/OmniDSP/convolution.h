/**
 * @file convolution.h
 * @brief Public API header for convolution and correlation functions in OmniDSP.
 *
...
namespace OmniDSP
{
     ...
     * (Note: Output size behavior might need adjustment based on desired mode like 'full' or 'same').
     *
     ...
     * @throws std::invalid_argument If signal or kernel is empty, or if kernel is longer than signal for 'valid' mode.
     */
    template <typename T>
    ...
    // The definitions that call the backend implementations should reside in a .cpp file.
    /** @cond OMNIDSP_INTERNAL */
    extern template std::vector<float> convolve1d<float>(const std::vector<float> &signal, const std::vector<float> &kernel);
    extern template std::vector<double> convolve1d<double>(const std::vector<double> &signal, const std::vector<double> &kernel);

    extern template std::vector<float> correlate1d<float>(const std::vector<float> &signal, const std::vector<float> &kernel);
    extern template std::vector<double> correlate1d<double>(const std::vector<double> &signal, const std::vector<double> &kernel);
    /** @endcond */

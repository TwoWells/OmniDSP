...
# ==============================
# Detects and configures the Apple Accelerate framework backend. This script
# checks if the build is happening on an Apple platform and attempts to find the
# Accelerate framework.
#
# Variables Set (PARENT_SCOPE, if Accelerate found): - OMNIDSP_HAS_ACCELERATE
# (BOOL): TRUE if Accelerate is found. - ACCELERATE_COMPILE_DEFINITION (STRING):
# Compile definition ("USE_ACCELERATE"). - ACCELERATE_LINK_LIB (STRING): Link
# library/framework name ("Accelerate" or path). - ACCELERATE_BACKEND_SOURCES
# (LIST): List of source files for the Accelerate backend. -
# ACCELERATE_BACKEND_INCLUDE_DIR (STRING): Path to the Accelerate backend's
...
set(OMNIDSP_HAS_ACCELERATE
    FALSE
    PARENT_SCOPE)

# Accelerate is only available on Apple platforms
if(APPLE)
  find_package(Accelerate QUIET) # Find the Accelerate framework using CMake's
                                 # built-in module
  ...
  if(Accelerate_FOUND OR CMAKE_SYSTEM_NAME STREQUAL "Darwin")
    ...
    set(ACCELERATE_COMPILE_DEFINITION
        "USE_ACCELERATE"
        PARENT_SCOPE)
    message(STATUS "    Compile definition: ${ACCELERATE_COMPILE_DEFINITION}")

    # Define the link library (framework name) find_library is good practice but
    # often linking "-framework Accelerate" works directly
    find_library(ACCELERATE_FRAMEWORK_PATH Accelerate)
    if(ACCELERATE_FRAMEWORK_PATH)
      set(ACCELERATE_LINK_LIB
          ${ACCELERATE_FRAMEWORK_PATH}
          PARENT_SCOPE)
      message(
        STATUS "    Found Accelerate framework at: ${ACCELERATE_FRAMEWORK_PATH}"
      )
    else()
      ...
      message(
        WARNING
          "    Could not find Accelerate framework path via find_library, will link by name 'Accelerate'."
      )
    endif()
    ...
    set(ACCELERATE_BACKEND_SOURCES
        ...
        PARENT_SCOPE)
    message(
      STATUS "    Accelerate backend sources: ${ACCELERATE_BACKEND_SOURCES}")
    ...
    message(
      STATUS
        "    Accelerate backend include dir: ${ACCELERATE_BACKEND_INCLUDE_DIR}")

...
# Ensure GNUInstallDirs has been included in the main CMakeLists.txt to have standard variables like
# CMAKE_INSTALL_LIBDIR, CMAKE_INSTALL_INCLUDEDIR, etc.
...
if(OMNIDSP_BUILD_PYTHON_BINDINGS)
  ...
  if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/examples/notebooks")
    ...
    message(
      STATUS "    Python examples directory 'examples/notebooks' not found, skipping installation.")
  endif()
  ...
  # Install the generated export header Assumes generate_export_header was called with
  # EXPORT_FILE_NAME OmniDSP/omnidsp_export.h
  install(
  ...
  if(NOT EXISTS "${CONFIG_CMAKE_IN_PATH}")
    ...
    # find_dependency(Boost REQUIRED) find_dependency(highway CONFIG REQUIRED) These should match
    # the PUBLIC dependencies in target_link_libraries.
  endif()

  # Generate the Version file (<Package>ConfigVersion.cmake) This helps find_package determine
  # version compatibility.
  write_basic_package_version_file(
    "${CMAKE_CURRENT_BINARY_DIR}/${PROJECT_NAME}ConfigVersion.cmake" # Output path in build dir
  ...
  # Configure the main package file (<Package>Config.cmake) from the template This substitutes
  # variables like @PACKAGE_INIT@ and configured PATH_VARS.
  ...
  # Install the library target (omnidsp) itself This installs the actual library files (.a, .lib,
  # .so, .dylib, .dll) It also defines an "export set" named OmniDSPTargets for the targets file.
  install(
  ...
  # Install the export set file (<TargetsName>.cmake) This file contains information about the
  # installed targets (location, properties) allowing other CMake projects to use them via
  # find_package.
  install(

...
int main() {
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        Real imag_val = std::sin(1.5 * M_PI * frequency * static_cast<Real>(i) / N); // Different frequency for imag part
        input_signal_c2c[i] = Complex(real_val, imag_val);
    }

    std::cout << "Input Complex Signal:" << std::endl;
    for (size_t i = 0; i < std::min((size_t)5, N); ++i) std::cout << input_signal_c2c[i] << " ";
    if (N > 5) std::cout << "...";
    ...
    try {
        ...
        OmniDSP::FFTPlan<Real> forward_plan_c2c(N, CURRENT_PRECISION, OmniDSP::Direction::FORWARD, OmniDSP::Domain::COMPLEX, OmniDSP::NormMode::BACKWARD);
        OmniDSP::FFTPlan<Real> inverse_plan_c2c(N, CURRENT_PRECISION, OmniDSP::Direction::INVERSE, OmniDSP::Domain::COMPLEX, OmniDSP::NormMode::BACKWARD);
        ...
        std::cerr << "\nFATAL ERROR during C2C FFT operation: " << e.what() << std::endl;
        return 1;
    }

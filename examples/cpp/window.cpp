 ...
 */
 ...
 int main() {
     ...
     try {
         ...
         double sample_rate = static_cast<double>(signal_length); // Make one cycle fit
         for (size_t i = 0; i < signal_length; ++i) {
             input_signal[i] = std::sin(2.0 * M_PI * freq * static_cast<double>(i) / sample_rate);
         }
         ...
         std::cout << "Applying Kaiser Window (beta=" << kaiser_beta << ")..." << std::endl;
         std::vector<Real> kaiser_output = OmniDSP::Window::kaiser(input_signal, kaiser_beta);

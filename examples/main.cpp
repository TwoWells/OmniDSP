/**
 * @file main.cpp
 * @brief Example usage of the OmniFFT library.
 *
...
#include <OmniFFT/omnifft.h> // Library header
...
int main() {
    ...
    const auto CURRENT_PRECISION = OmniFFT::Precision::DOUBLE; // Match Precision enum to Real type

    std::cout << "OmniFFT Example (N=" << N << ")" << std::endl;
    std::cout << "-----------------------------" << std::endl;
    ...
    // --- Execute FFTs within a try-catch block ---
    try {
        // ==========================================================
        ...
        OmniFFT::FFTPlan<Real> forward_plan_c2c(N, CURRENT_PRECISION, OmniFFT::Direction::FORWARD, OmniFFT::Domain::COMPLEX, OmniFFT::NormMode::BACKWARD);
        OmniFFT::FFTPlan<Real> inverse_plan_c2c(N, CURRENT_PRECISION, OmniFFT::Direction::INVERSE, OmniFFT::Domain::COMPLEX, OmniFFT::NormMode::BACKWARD);
        ...
        for(size_t i=0; i<std::min((size_t)5, N); ++i) std::cout << spectrum_c2c_plan[i] << " ";
        if (N>5) std::cout << "...";
        std::cout << std::endl;
        ...
        for(size_t i=0; i<std::min((size_t)5, N); ++i) std::cout << reconstructed_c2c_plan[i] << " ";
         if (N>5) std::cout << "...";
        std::cout << std::endl;

        // Verification (optional, needs scaling consideration for non-MKL backends/norms)
        Real max_diff_oop = 0.0;
        for(size_t i=0; i<N; ++i) max_diff_oop = std::max(max_diff_oop, std::abs(input_signal_c2c[i] - reconstructed_c2c_plan[i]));
        std::cout << "Max Difference (OOP): " << max_diff_oop << std::endl << std::endl;
        ...
        // ==========================================================
         std::cout << "--- Method 2: Using Convenience Functions (C2C OOP) ---" << std::endl;
        ...
        OmniFFT::fft(input_signal_c2c, spectrum_conv_c2c);   // Forward
        OmniFFT::ifft(spectrum_conv_c2c, reconstructed_conv_c2c); // Inverse

        std::cout << "C2C Spectrum (Convenience Func):" << std::endl;
        for(size_t i=0; i<std::min((size_t)5, N); ++i) std::cout << spectrum_conv_c2c[i] << " ";
        if (N>5) std::cout << "...";
        std::cout << std::endl;

        std::cout << "\nC2C Reconstructed (Convenience Func):" << std::endl;
        for(size_t i=0; i<std::min((size_t)5, N); ++i) std::cout << reconstructed_conv_c2c[i] << " ";
        if (N>5) std::cout << "...";
        std::cout << std::endl << std::endl;
        ...
        // ==========================================================
         std::cout << "--- Method 3: Using In-place Transform (C2C) ---" << std::endl;
         std::vector<Complex> data_inplace_c2c = input_signal_c2c; // Copy input data

         // Use convenience function for simplicity
         OmniFFT::fft_inplace(data_inplace_c2c); // Forward transform in-place

         std::cout << "C2C Spectrum (In-place Forward):" << std::endl;
         for(size_t i=0; i<std::min((size_t)5, N); ++i) std::cout << data_inplace_c2c[i] << " ";
         if (N>5) std::cout << "...";
         std::cout << std::endl;

         OmniFFT::ifft_inplace(data_inplace_c2c); // Inverse transform in-place
         ...
         for(size_t i=0; i<N; ++i) max_diff_ip = std::max(max_diff_ip, std::abs(input_signal_c2c[i] - data_inplace_c2c[i]));
         std::cout << "Max Difference (In-Place): " << max_diff_ip << std::endl << std::endl;
        ...
        OmniFFT::rfft(input_signal_real, rfft_spectrum);
        ...
        OmniFFT::irfft(rfft_spectrum, irfft_reconstructed);

        std::cout << "\nIRFFT Reconstructed Signal (Size " << irfft_reconstructed.size() << "):" << std::endl;
        // Note: Manual scaling might be needed here depending on backend/norm mode if not BACKWARD.
        // The convenience functions use BACKWARD, where MKL scales by 1/N and Accelerate impl attempts internal scaling.
        ...
        std::cout << "Max Difference (Real): " << max_diff_real << std::endl << std::endl;

        // ==========================================================
        // Example: Using ORTHO normalization via FFTPlan
        // ==========================================================
        std::cout << "--- Example: Using FFTPlan with ORTHO Norm (RFFT/IRFFT) ---" << std::endl;
        try {
            OmniFFT::FFTPlan<Real> plan_rfft_ortho(N, CURRENT_PRECISION, OmniFFT::Direction::FORWARD, OmniFFT::Domain::REAL, OmniFFT::NormMode::ORTHO);
            OmniFFT::FFTPlan<Real> plan_irfft_ortho(N, CURRENT_PRECISION, OmniFFT::Direction::INVERSE, OmniFFT::Domain::REAL, OmniFFT::NormMode::ORTHO);
    ...
    std::cout << "OmniFFT Example Finished." << std::endl;
    return 0;
}